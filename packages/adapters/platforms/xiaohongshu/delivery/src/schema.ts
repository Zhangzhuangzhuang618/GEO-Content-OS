...
import { XiaohongshuPayloadSchema } from '../../render/src/schema.js';

export const XiaohongshuDeliveryInputSchema = z
  .object({
    content_version_id: z.string().uuid(),
    idempotency_key: z.string().regex(/^[A-Za-z0-9._:-]{1,128}$/u),
    payload: XiaohongshuPayloadSchema,
    payload_hash: z.string().regex(/^[a-f0-9]{64}$/u),
  })
  .strict();
export const XiaohongshuCapabilityResponseSchema = z
  .object({ get_status: z.boolean(), metrics: z.boolean(), publish: z.boolean() })
  .strict();
export const XiaohongshuPublishResponseSchema = z
  .object({
  ...
  .strict();
export const XiaohongshuStatusResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
  ...
  .strict();
export const XiaohongshuMetricsResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
