...
import { SohuPayloadSchema } from '../../render/src/schema.js';

export const SohuDeliveryInputSchema = z
  .object({
    content_version_id: z.string().uuid(),
    idempotency_key: z.string().regex(/^[A-Za-z0-9._:-]{1,128}$/u),
    payload: SohuPayloadSchema,
    payload_hash: z.string().regex(/^[a-f0-9]{64}$/u),
  })
  .strict();

export const SohuCapabilityResponseSchema = z
  .object({
...
export const SohuPublishResponseSchema = z
  .object({
...
export const SohuStatusResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
...
export const SohuMetricsResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
