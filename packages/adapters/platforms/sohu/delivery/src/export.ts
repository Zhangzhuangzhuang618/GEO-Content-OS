...
import { SohuDeliveryInputSchema } from './schema.js';
import {
...
export function hashSohuPayload(payload: SohuDeliveryInput['payload']): string {
  return sha256(stableStringify(payload));
}

export function exportSohu(input: unknown): SohuExportBundle {
  const parsed = SohuDeliveryInputSchema.parse(input) as SohuDeliveryInput;
  const actualHash = hashSohuPayload(parsed.payload);
  ...
  const metadata = {
    ...
    ai_generated: parsed.payload.ai_generated,
    original: parsed.payload.original,
    platform_code: 'sohu',
  ...
  const manifestBody = `${stableStringify({
    ...
    platform_code: 'sohu',
    rule_version: parsed.payload.rule_version,
    schema_version: SOHU_EXPORT_SCHEMA_VERSION,
  })}\n`;
...
function file(path: string, contentType: string, body: string): SohuExportFile {
