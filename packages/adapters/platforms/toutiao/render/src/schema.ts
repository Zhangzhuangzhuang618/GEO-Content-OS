...
export const ToutiaoContentSchema = z
  .object({
    ...
    platform_code: z.literal(TOUTIAO_PLATFORM_CODE),
    platform_meta: ToutiaoPlatformMetaSchema,
    summary: z.string().max(240),
    title: unicodeText(2, 80),
  })
  .strict();

export const ToutiaoCitationLinkSchema = z
  .object({
...
export const ToutiaoRenderInputSchema = z
  .object({
    citations: z
      .array(ToutiaoCitationLinkSchema)
      .max(200)
      .refine((items) => unique(items.map((item) => item.citation_id))),
    content: ToutiaoContentSchema,
    rule_version: z.literal(TOUTIAO_RENDER_RULE_VERSION),
  })
  .strict();

export const ToutiaoPayloadSchema = z
  .object({
    body_html: z.string().min(1),
    body_text: z.string().min(1),
    citation_links: z.array(ToutiaoCitationLinkSchema).max(200),
    content_type: z.string().trim().min(1).max(40),
    lead: unicodeText(1, 100),
    platform_code: z.literal(TOUTIAO_PLATFORM_CODE),
    rule_version: z.literal(TOUTIAO_RENDER_RULE_VERSION),
    schema_version: z.literal(TOUTIAO_PAYLOAD_SCHEMA_VERSION),
    tags: z.array(z.string().trim().min(1).max(40)).max(20).refine(unique),
    title: unicodeText(2, 50),
  })
  .strict();

export const TOUTIAO_RENDER_INPUT_JSON_SCHEMA = Object.freeze({
  $id: 'https://geo.example/schemas/toutiao-render-input-1.json',
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  additionalProperties: false,
  properties: {
    citations: { items: { $ref: '#/$defs/citation' }, maxItems: 200, type: 'array' },
    content: { $ref: '#/$defs/content' },
    rule_version: { const: TOUTIAO_RENDER_RULE_VERSION },
  },
...
export const TOUTIAO_PAYLOAD_JSON_SCHEMA = Object.freeze({
  $id: 'https://geo.example/schemas/toutiao-payload-1.json',
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  additionalProperties: false,
  properties: {
    ...
    lead: { maxLength: 100, minLength: 1, type: 'string' },
    platform_code: { const: TOUTIAO_PLATFORM_CODE },
    rule_version: { const: TOUTIAO_RENDER_RULE_VERSION },
    schema_version: { const: TOUTIAO_PAYLOAD_SCHEMA_VERSION },
    tags: {
    ...
    title: { maxLength: 50, minLength: 2, type: 'string' },
  },
  required: [
    'schema_version',
    'rule_version',
    ...
    content: {
      ...
      properties: {
        ...
        platform_meta: {
          ...
          properties: {
            content_type: { maxLength: 40, minLength: 1, type: 'string' },
            lead: { maxLength: 240, type: 'string' },
            tags: {
              items: { maxLength: 40, minLength: 1, type: 'string' },
              maxItems: 20,
              type: 'array',
          ...
          required: ['lead', 'tags', 'content_type'],
          type: 'object',
        },
