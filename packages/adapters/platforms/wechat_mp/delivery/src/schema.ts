...
import { WechatMpPayloadSchema } from '../../render/src/schema.js';

export const WechatMpDeliveryInputSchema = z
  .object({
    content_version_id: z.string().uuid(),
    idempotency_key: z.string().regex(/^[A-Za-z0-9._:-]{1,128}$/u),
    payload: WechatMpPayloadSchema,
    payload_hash: z.string().regex(/^[a-f0-9]{64}$/u),
  })
  .strict();
export const WechatMpCapabilityResponseSchema = z
  .object({ get_status: z.boolean(), metrics: z.boolean(), publish: z.boolean() })
  .strict();
export const WechatMpPublishResponseSchema = z
  .object({
  ...
  .strict();
export const WechatMpStatusResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
  ...
  .strict();
export const WechatMpMetricsResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
