...
import { LiejuDeliveryInputSchema } from './schema.js';
import {
...
export function hashLiejuPayload(payload: LiejuDeliveryInput['payload']): string {
  return sha256(stableStringify(payload));
}

export function exportLieju(input: unknown): LiejuExportBundle {
  const parsed = LiejuDeliveryInputSchema.parse(input) as LiejuDeliveryInput;
  const actualHash = hashLiejuPayload(parsed.payload);
  ...
  const contentFiles = [
    file(`${basePath}/classified.txt`, 'text/plain; charset=utf-8', parsed.payload.body_text),
  ...
  const manifestBody = `${stableStringify({
    ...
    platform_code: 'lieju',
    rule_version: parsed.payload.rule_version,
    schema_version: LIEJU_EXPORT_SCHEMA_VERSION,
  })}\n`;
...
function file(path: string, contentType: string, body: string): LiejuExportFile {
