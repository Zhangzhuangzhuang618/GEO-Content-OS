...
import { BaijiahaoPayloadSchema } from '../../render/src/schema.js';

export const BaijiahaoDeliveryInputSchema = z
  .object({
    content_version_id: z.string().uuid(),
    idempotency_key: z.string().regex(/^[A-Za-z0-9._:-]{1,128}$/u),
    payload: BaijiahaoPayloadSchema,
    payload_hash: z.string().regex(/^[a-f0-9]{64}$/u),
  })
  .strict();

export const BaijiahaoCapabilityResponseSchema = z
  .object({
...
export const BaijiahaoPublishResponseSchema = z
  .object({
...
export const BaijiahaoStatusResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
...
export const BaijiahaoMetricsResponseSchema = z
  .object({
    external_id: z.string().trim().min(1).max(240),
