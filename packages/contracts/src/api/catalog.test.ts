...
describe('API contract catalog', () => {
  it('contains the 143-operation executable ADR baseline without duplicates', () => {
    const routes = API_CONTRACTS.map((contract) => `${contract.method} ${contract.path}`);
    expect(routes).toHaveLength(143);
    expect(new Set(routes).size).toBe(143);
    expect(new Set(API_CONTRACTS.map((contract) => contract.key)).size).toBe(143);
  });
