...
HOTFIX-20260819-06：证照扫描件改用独立输入门槛，允许最长边 8192 像素、总像素不超过
5000 万且文件不超过 10 MB；普通生成配图门槛不变，证照发布副本继续自动缩为 1200×800。

HOTFIX-20260819-07：上述高分辨率输入门槛扩展到所有导入资料图片；普通图片保留 25 MiB 上限，
证照核验字段、10 MB 上限和受控发布规则不变。
