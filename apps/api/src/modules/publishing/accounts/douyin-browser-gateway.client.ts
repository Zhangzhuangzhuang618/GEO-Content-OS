...
export class DouyinBrowserGatewayClient {
  private readonly baseUrl: string;
  private readonly token: string | null;
  private readonly timeoutMs: number;

  public constructor(environment: NodeJS.ProcessEnv = process.env) {
    const configuration = readDouyinBrowserGatewayCredential(environment, false);
  ...
  private async request(path: string, method: 'GET' | 'POST'): Promise<unknown> {
    if (!this.token || this.token.length < 32) {
      throw new PlatformAccountError(
        ...
        'Douyin browser gateway is not configured',
      );
    }
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new PlatformAccountError(
            ...
            'Douyin browser session was not found',
          );
        }
...
function readGatewayTimeout(environment: NodeJS.ProcessEnv): number {
  const value = Number(environment['DOUYIN_BROWSER_GATEWAY_TIMEOUT_MS']?.trim() || 65_000);
  if (!Number.isInteger(value) || value < 1_000 || value > 120_000) {
    throw new Error('DOUYIN_BROWSER_GATEWAY_TIMEOUT_MS is invalid');
  }
  return value;
}

export function readDouyinBrowserGatewayCredential(
  environment: NodeJS.ProcessEnv = process.env,
  requireToken = true,
): { readonly base_url: string; readonly bearer_token: string } {
  const baseUrl =
    environment['DOUYIN_BROWSER_GATEWAY_BASE_URL']?.trim() || 'http://douyin-browser:9098';
  const parsed = new URL(baseUrl);
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error('DOUYIN_BROWSER_GATEWAY_BASE_URL is invalid');
  }
  const token = environment['DOUYIN_BROWSER_GATEWAY_TOKEN']?.trim() || '';
...
function unavailable(): PlatformAccountError {
  return new PlatformAccountError(
    'PLATFORM_ACCOUNT_STATE_INVALID',
    'Douyin browser gateway is unavailable',
...
async function upstreamFailure(response: Response): Promise<PlatformAccountError> {
  ...
  return new PlatformAccountError(
    'PLATFORM_ACCOUNT_STATE_INVALID',
    'Douyin browser gateway rejected the operation',
