      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new PlatformAccountError(
            ...
            'Sohu browser session was not found',
          );
        }
...
function readGatewayTimeout(environment: NodeJS.ProcessEnv): number {
  const value = Number(environment['SOHU_BROWSER_GATEWAY_TIMEOUT_MS']?.trim() || 65_000);
  if (!Number.isInteger(value) || value < 1_000 || value > 120_000) {
    throw new Error('SOHU_BROWSER_GATEWAY_TIMEOUT_MS is invalid');
  }
  return value;
}

export function readSohuBrowserGatewayCredential(
  environment: NodeJS.ProcessEnv = process.env,
  requireToken = true,
): { readonly base_url: string; readonly bearer_token: string } {
  const baseUrl =
    environment['SOHU_BROWSER_GATEWAY_BASE_URL']?.trim() || 'http://sohu-browser:9096';
  const parsed = new URL(baseUrl);
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error('SOHU_BROWSER_GATEWAY_BASE_URL is invalid');
  }
  const token = environment['SOHU_BROWSER_GATEWAY_TOKEN']?.trim() || '';
...
function unavailable(): PlatformAccountError {
  return new PlatformAccountError(
    'PLATFORM_ACCOUNT_STATE_INVALID',
    'Sohu browser gateway is unavailable',
...
async function upstreamFailure(response: Response): Promise<PlatformAccountError> {
  ...
  return new PlatformAccountError(
    'PLATFORM_ACCOUNT_STATE_INVALID',
    'Sohu browser gateway rejected the operation',
