...
RUN pnpm --filter @geo-content-os/worker-baijiahao-browser build
...
RUN mkdir -p /var/lib/geo-content-os/baijiahao && \
    chown -R pwuser:pwuser /var/lib/geo-content-os

USER pwuser
EXPOSE 9095
CMD ["node", "workers/baijiahao-browser/dist/main.js"]
