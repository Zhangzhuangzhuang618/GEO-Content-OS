...
export class PostgresBaijiahaoBrowserStore {
  ...
  public async getOrCreateSession(accountId: string): Promise<BrowserSession> {
    return this.client.begin(async (transaction) => {
      ...
      const profileKey = `baijiahao/${account.tenantId}/${account.id}`;
      await transaction`
  ...
  public async markSession(
    ...
    const rows = await this.client<BrowserSession[]>`
      UPDATE baijiahao_browser_sessions SET
        status=${input.status},
  ...
  public async markAccountReauth(accountId: string, tenantId: string): Promise<void> {
    await this.client`
      ...
      WHERE id=${accountId}::uuid AND tenant_id=${tenantId}::uuid
        AND platform_code='baijiahao' AND status='active' AND deleted_at IS NULL
    `;
  }
  ...
  public async markAccountActive(accountId: string, tenantId: string): Promise<void> {
    await this.client`
      ...
      WHERE id=${accountId}::uuid AND tenant_id=${tenantId}::uuid
        AND platform_code='baijiahao' AND status='reauth' AND deleted_at IS NULL
    `;
  }
  ...
  public async preparePublication(
    ...
    return this.client.begin(async (transaction) => {
      const account = await requireAccount(transaction, accountId, true);
      if (account.status !== 'active') throw invalid('Baijiahao account requires authentication');
      const sessions = await selectSession(transaction, accountId, true);
      const session = sessions[0];
      if (!session || session.status !== 'authenticated') {
        throw invalid('Baijiahao browser session is not authenticated');
      }
      const jobs = await transaction<{ id: string }[]>`
        ...
        WHERE tenant_id=${account.tenantId}::uuid AND account_id=${accountId}::uuid
          ...
          AND origin IN ('manual','baijiahao_automation') AND status='publishing'
        FOR UPDATE
      `;
      const job = jobs[0];
      if (!job) throw invalid('No matching Baijiahao publish job is currently publishing');
      const fieldSummary = Object.freeze({
        ...
        tag_count: input.payload.tags.length,
  ...
  public async loadImageAssets(
    ...
    if (new Set(references.map(({ assetId }) => assetId)).size !== references.length) {
      throw invalid('A Baijiahao image asset cannot be used more than once');
    }
    ...
    >`
      ...
      FROM baijiahao_browser_publications AS publication
    ...
    if (byId.size !== references.length) {
      throw invalid(
        'Baijiahao images must belong to this content version and have a verified no-watermark result',
      );
    }
    return Object.freeze(
      references.map(({ assetId, role }) => {
        const row = byId.get(assetId);
        if (!row) throw invalid('Baijiahao image asset scope is invalid');
        return Object.freeze({
          assetId,
  ...
  public async insertArtifact(
    ...
    await this.client`
      INSERT INTO baijiahao_browser_artifacts (
...
async function requireAccount(
  ...
  const rows = await sql<AccountRow[]>`
    ...
    WHERE id=${accountId}::uuid AND platform_code='baijiahao' AND deleted_at IS NULL
    ${lock ? sql`FOR UPDATE` : sql``}
  `;
  const account = rows[0];
  if (!account) throw notFound('Baijiahao account does not exist');
  return account;
}
