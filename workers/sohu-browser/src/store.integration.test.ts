...
import { PostgresSohuBrowserStore } from './store.js';
...
describe('Postgres Sohu browser store', () => {
  ...
  beforeEach(async () => {
    ...
    await database`
      TRUNCATE
        sohu_browser_artifacts,sohu_browser_publications,sohu_browser_sessions,
  ...
  it('persists encrypted session state and enforces publication idempotency', async () => {
    const store = new PostgresSohuBrowserStore(requireClient(client));
    const session = await store.getOrCreateSession(ACCOUNT_ID);
    expect(session.profileKey).toBe(`sohu/${TENANT_ID}/${ACCOUNT_ID}`);
    ...
    await expect(store.loadImageAssets(publication, COVER_ID, [])).resolves.toEqual([
      {
        ...
        mimeType: 'image/png',
        objectUri: 's3://test/sohu/cover.png',
        role: 'cover',
        sizeBytes: 145,
      },
    ]);
...
function publishInput(): BrowserPublishInput {
  return {
    ...
    idempotencyKey: 'sohu-t150-publication',
    payload: {
      abstract: '这是用于浏览器存储集成测试的摘要。',
      body_asset_ids: [],
      body_html: '<p>搜狐号存储集成测试正文。</p>',
      body_text: '搜狐号存储集成测试正文。',
      citation_links: [],
      content_type: 'news',
...
async function seed(database: Sql): Promise<void> {
  await database`
    ...
    VALUES(${USER_ID}::uuid,'sohu-t150@example.com','T150','active')
  `;
  await database`
  ...
  await database`
    INSERT INTO briefs(
      ...
      'T150 Brief','education','企业内容发布管理人员',ARRAY['sohu']::varchar[],
  ...
  await database`
    ...
    VALUES(${VARIANT_ID}::uuid,${TENANT_ID}::uuid,${PACKAGE_ID}::uuid,'sohu','publishing')
  `;
  await database`
  ...
  await database`
    INSERT INTO platform_accounts(
      ...
      ${ACCOUNT_ID}::uuid,${TENANT_ID}::uuid,${WORKSPACE_ID}::uuid,'sohu',
      't150-provider','T150 Sohu','{"publish":true}'::jsonb,'api','active','Asia/Shanghai'
    )
  `;
  await database`
    INSERT INTO publish_jobs(
  ...
  await database`
    INSERT INTO media_assets(
      ...
      (
        ...
        'image','s3://test/sohu/cover.png',${COVER_HASH},'image/png',145,
      ...
      (
        ...
        'image','s3://test/sohu/unverified.png',${'c'.repeat(64)},'image/png',145,
