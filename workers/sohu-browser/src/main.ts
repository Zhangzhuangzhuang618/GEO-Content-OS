...
import { createBrowserCredentialService, readSohuBrowserConfig } from './config.js';
import { PlaywrightSohuPageDriver } from './page-driver.js';
import { createGatewayServer } from './server.js';
import { SohuBrowserService } from './service.js';
...
async function main(): Promise<void> {
  ...
  const driver = new PlaywrightSohuPageDriver(config);
  const service = new SohuBrowserService(
    config,
    new PostgresSohuBrowserStore(database),
    driver,
