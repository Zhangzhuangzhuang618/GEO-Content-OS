...
import { z } from 'zod';
import { SohuBrowserLoginRequestSchema } from '@geo-content-os/contracts';
...
export function createGatewayServer(service: SohuBrowserService, isReady: () => boolean): Server {
...
async function loginBody(request: IncomingMessage) {
  ...
  if (contentLength === '0' || (contentLength === undefined && transferEncoding === undefined)) {
    return SohuBrowserLoginRequestSchema.parse(undefined);
  }
  const parsed = SohuBrowserLoginRequestSchema.safeParse(await jsonBody(request));
  if (!parsed.success) {
...
function logGatewayFailure(
  ...
  console.error('Sohu browser gateway request failed', {
