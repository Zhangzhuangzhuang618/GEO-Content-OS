...
export interface PublicationRow extends PublicationClaim {
...
export class PostgresDouyinBrowserStore {
  ...
  public async getOrCreateSession(accountId: string): Promise<BrowserSession> {
    return this.client.begin(async (transaction) => {
      ...
      const profileKey = `douyin/${account.tenantId}/${account.id}`;
      await transaction`
        INSERT INTO douyin_browser_sessions (tenant_id, account_id, status, profile_key)
        VALUES (
      ...
      `;
      const session = (await selectSession(transaction, account.id, true))[0];
      if (!session) throw invalid('Douyin browser session was not created');
  ...
  public async getSession(accountId: string): Promise<BrowserSession> {
    ...
    const session = (await selectSession(this.client, accountId, false))[0];
    if (!session) throw notFound('Douyin browser session does not exist');
    return Object.freeze(session);
  }
  ...
  public async markAccountReauth(accountId: string, tenantId: string): Promise<void> {
    await this.client`
      ...
      WHERE id=${accountId}::uuid AND tenant_id=${tenantId}::uuid
        AND platform_code='douyin' AND status='active' AND deleted_at IS NULL
    `;
  }
  ...
  public async markAccountActive(accountId: string, tenantId: string): Promise<void> {
    await this.client`
      ...
      WHERE id=${accountId}::uuid AND tenant_id=${tenantId}::uuid
        AND platform_code='douyin' AND status='reauth' AND deleted_at IS NULL
    `;
  }
  ...
  public async preparePublication(
    ...
    return this.client.begin(async (transaction) => {
      const account = await requireAccount(transaction, accountId, true);
      if (account.status !== 'active') throw invalid('Douyin account requires authentication');
      const session = (await selectSession(transaction, accountId, true))[0];
      if (!session || session.status !== 'authenticated') {
        throw invalid('Douyin browser session is not authenticated');
      }
      const job = (
        await transaction<{ id: string }[]>`
          ...
          WHERE tenant_id=${account.tenantId}::uuid AND account_id=${accountId}::uuid
            ...
            AND origin IN ('manual','douyin_automation') AND status='publishing'
          FOR UPDATE
        `
      )[0];
      if (!job) throw invalid('No matching Douyin publish job is currently publishing');
      const fieldSummary = Object.freeze({
        ai_generated: input.payload.ai_generated,
        card_count: input.payload.cards.length,
        content_kind: input.payload.content_kind,
        description_characters: Array.from(input.payload.description).length,
        title_characters: Array.from(input.payload.title).length,
        topic_count: input.payload.topics.length,
      });
      await transaction`
      ...
      `;
      const publication = (
        await selectPublication(transaction, account.tenantId, accountId, input.idempotencyKey)
      )[0];
      if (!publication) throw invalid('Douyin browser publication was not created');
      const frozen = (
        await transaction<{ contentVersionId: string; payloadHash: string }[]>`
          SELECT content_version_id AS "contentVersionId", payload_hash AS "payloadHash"
      ...
      if (
        ...
        throw conflict('Idempotency key has different frozen Douyin content');
      }
  ...
  public async updatePublication(
    ...
    const rows = await this.client<PublicationRow[]>`
      UPDATE douyin_browser_publications SET
        ...
        submitted_at=${input.submittedAt === undefined ? this.client`submitted_at` : input.submittedAt},
      ...
      RETURNING id, tenant_id AS "tenantId", session_id AS "sessionId",
        ...
        external_post_id AS "externalId", external_url AS "externalUrl",
        review_reason AS "reviewReason", submitted_at AS "submittedAt", version
    `;
    const updated = rows[0];
    if (!updated) throw conflict('Douyin browser publication changed concurrently');
    return Object.freeze(updated);
  }
    ...
    const rows = await this.client<PublicationRow[]>`
      ...
      WHERE tenant_id=${account.tenantId}::uuid AND account_id=${accountId}::uuid
        AND (
          ...
          OR content_fingerprint=${externalId}
          OR id::text=${externalId}
        )
    `;
    const publication = rows[0];
    if (!publication) throw notFound('Douyin browser publication does not exist');
    return Object.freeze(publication);
  }
  ...
  public async loadImageAssets(
    ...
    if (imageAssetIds.length < 5 || imageAssetIds.length > 10) {
      throw invalid('Douyin image notes require 5 to 10 ordered images');
    }
    if (new Set(imageAssetIds).size !== imageAssetIds.length) {
      throw invalid('A Douyin image asset cannot be used more than once');
    }
    const ids = this.client.array([...imageAssetIds], 2950);
    ...
    >`
      SELECT asset.id AS "assetId", asset.object_uri AS "objectUri",
        asset.content_hash AS "contentHash", asset.mime_type AS "mimeType",
        asset.size_bytes::text AS "sizeBytes"
      FROM douyin_browser_publications AS publication
    ...
    if (byId.size !== imageAssetIds.length) {
      throw invalid(
        'Douyin images must belong to this content version and have a verified no-watermark result',
      );
    }
    return Object.freeze(
      imageAssetIds.map((assetId) => {
        const row = byId.get(assetId);
        if (!row) throw invalid('Douyin image asset scope is invalid');
        return Object.freeze({ ...row, sizeBytes: Number(row.sizeBytes) });
      }),
    );
  }
  ...
  public async insertArtifact(
    ...
    await this.client`
      INSERT INTO douyin_browser_artifacts (
...
async function requireAccount(
  ...
  const account = (
    await sql<AccountRow[]>`
      ...
      WHERE id=${accountId}::uuid AND platform_code='douyin' AND deleted_at IS NULL
      ${lock ? sql`FOR UPDATE` : sql``}
    `
  )[0];
  if (!account) throw notFound('Douyin account does not exist');
  return account;
}
...
function conflict(message: string): BrowserStoreError {
  return new BrowserStoreError('CONFLICT', message);
}
