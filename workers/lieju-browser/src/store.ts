...
export class PostgresLiejuBrowserStore {
  ...
  public async getOrCreateSession(accountId: string): Promise<BrowserSession> {
    return this.client.begin(async (transaction) => {
      ...
      const profileKey = `lieju/${account.tenantId}/${account.id}`;
      await transaction`
        INSERT INTO lieju_browser_sessions (
  ...
  public async markSession(
    ...
    const rows = await this.client<BrowserSession[]>`
      UPDATE lieju_browser_sessions SET
        status=${input.status},
  ...
  public async markAccountReauth(accountId: string, tenantId: string): Promise<void> {
    await this.client`
      ...
      WHERE id=${accountId}::uuid AND tenant_id=${tenantId}::uuid
        AND platform_code='lieju' AND status='active' AND deleted_at IS NULL
    `;
  }
  ...
  public async markAccountActive(accountId: string, tenantId: string): Promise<void> {
    await this.client`
      ...
      WHERE id=${accountId}::uuid AND tenant_id=${tenantId}::uuid
        AND platform_code='lieju' AND status='reauth' AND deleted_at IS NULL
    `;
  }
  ...
  public async preparePublication(
    ...
    return this.client.begin(async (transaction) => {
      const account = await requireAccount(transaction, accountId, true);
      if (account.status !== 'active') throw invalid('Lieju account requires authentication');
      const sessions = await selectSession(transaction, accountId, true);
      const session = sessions[0];
      if (!session || session.status !== 'authenticated') {
        throw invalid('Lieju browser session is not authenticated');
      }
      ...
      `;
      const job = jobs[0];
      if (!job) throw invalid('No matching Lieju publish job is currently publishing');
      const fieldSummary = Object.freeze({
        ...
        category_id: input.postingProfile.category_id,
        zone_id: input.postingProfile.zone_id,
        title_characters: Array.from(input.payload.title).length,
      });
      await transaction`
        INSERT INTO lieju_browser_publications (
  ...
  public async loadImageAssets(
    ...
    if (new Set(references.map(({ assetId }) => assetId)).size !== references.length) {
      throw invalid('A Lieju image asset cannot be used more than once');
    }
    ...
    >`
      ...
      FROM lieju_browser_publications AS publication
    ...
    if (byId.size !== references.length) {
      throw invalid(
        'Lieju images must belong to this content version and have a verified no-watermark result',
      );
    }
    return Object.freeze(
      references.map(({ assetId, role }) => {
        const row = byId.get(assetId);
        if (!row) throw invalid('Lieju image asset scope is invalid');
        return Object.freeze({
          assetId,
  ...
  public async insertArtifact(
    ...
    await this.client`
      INSERT INTO lieju_browser_artifacts (
...
function selectSession(
  ...
  return sql<BrowserSession[]>`
    ...
    FROM lieju_browser_sessions WHERE account_id=${accountId}::uuid
...
async function requireAccount(
  ...
  const rows = await sql<AccountRow[]>`
    ...
    WHERE id=${accountId}::uuid AND platform_code='lieju' AND deleted_at IS NULL
    ${lock ? sql`FOR UPDATE` : sql``}
  `;
  const account = rows[0];
  if (!account) throw notFound('Lieju account does not exist');
  return account;
}
