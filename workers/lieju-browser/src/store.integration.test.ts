...
describe('Postgres Lieju browser store', () => {
  ...
  beforeEach(async () => {
    ...
    await database`
      TRUNCATE
        lieju_browser_artifacts,lieju_browser_publications,lieju_browser_sessions,
        publish_attempts,publish_jobs,media_assets,
  ...
  it('persists encrypted session state and enforces publication idempotency', async () => {
    const store = new PostgresLiejuBrowserStore(requireClient(client));
    const session = await store.getOrCreateSession(ACCOUNT_ID);
    expect(session.profileKey).toBe(`lieju/${TENANT_ID}/${ACCOUNT_ID}`);
  ...
  it('accepts a Lieju automation publish job for browser submission', async () => {
    const database = requireClient(client);
    ...
    await database`
      INSERT INTO publish_jobs(
        ...
        ${ACCOUNT_ID}::uuid,now(),'lieju-t150-publication',${CONTENT_HASH},
        'publishing','lieju_automation',1,${USER_ID}::uuid
      )
    `;
    const store = new PostgresLiejuBrowserStore(database);
    const session = await store.getOrCreateSession(ACCOUNT_ID);
    ...
    await expect(store.loadImageAssets(publication, COVER_ID, [])).resolves.toEqual([
      {
        ...
        mimeType: 'image/png',
        objectUri: 's3://test/lieju/cover.png',
        role: 'cover',
        sizeBytes: 145,
      },
    ]);
...
function publishInput(): BrowserPublishInput {
  return {
    ...
    idempotencyKey: 'lieju-t150-publication',
    payload: {
      body_text: '列举网存储集成测试正文。'.repeat(50),
      citation_links: [],
...
async function seed(database: Sql): Promise<void> {
  await database`
    ...
    VALUES(${USER_ID}::uuid,'lieju-t150@example.com','T150','active')
  `;
  await database`
    INSERT INTO tenants(id,name,slug,status)
    VALUES(${TENANT_ID}::uuid,'T150 Tenant','t150-tenant','active')
  `;
  await database`
  ...
  await database`
    ...
    VALUES(${WORKSPACE_ID}::uuid,${TENANT_ID}::uuid,'T150','t150','Asia/Shanghai','active')
  `;
  await database`
  ...
  await database`
    ...
    VALUES(${PROJECT_ID}::uuid,${TENANT_ID}::uuid,${WORKSPACE_ID}::uuid,'T150',${USER_ID}::uuid,'active')
  `;
  await database`
    INSERT INTO briefs(
      ...
      'T150 Brief','education','企业内容发布管理人员',ARRAY['lieju']::varchar[],
  ...
  await database`
    ...
    VALUES(${VARIANT_ID}::uuid,${TENANT_ID}::uuid,${PACKAGE_ID}::uuid,'lieju','publishing')
  `;
  await database`
  ...
  await database`
    INSERT INTO platform_accounts(
      ...
      ${ACCOUNT_ID}::uuid,${TENANT_ID}::uuid,${WORKSPACE_ID}::uuid,'lieju',
      't150-provider','T150 Lieju','{"publish":true}'::jsonb,'api','active','Asia/Shanghai'
    )
  `;
  await database`
    INSERT INTO publish_jobs(
  ...
  await database`
    INSERT INTO media_assets(
      ...
      (
        ...
        'image','s3://test/lieju/cover.png',${COVER_HASH},'image/png',145,
      ...
      (
        ...
        'image','s3://test/lieju/unverified.png',${'c'.repeat(64)},'image/png',145,
