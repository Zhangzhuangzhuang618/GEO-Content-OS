...
describe('Lieju browser gateway routes', () => {
  ...
  it('passes the Adapter idempotency header into the frozen browser request', async () => {
    ...
    const response = await fetch(`${baseUrl}/publish`, {
      ...
      headers: {
        ...
        'idempotency-key': 'lieju:variant:version',
    ...
    expect(publish).toHaveBeenCalledWith(ACCOUNT_ID, {
      ...
      idempotency_key: 'lieju:variant:version',
    });
  });
  ...
  it('logs the redacted browser stage while keeping the public 503 generic', async () => {
    ...
    const response = await fetch(`${baseUrl}/publish`, {
      ...
      headers: {
        authorization: `Bearer ${TOKEN}`,
        'content-type': 'application/json',
        'idempotency-key': 'lieju:diagnostic:test',
        'x-platform-account-id': ACCOUNT_ID,
      },
      method: 'POST',
    });
    ...
    expect(errorLog).toHaveBeenCalledWith(
      'Lieju browser gateway request failed',
      expect.objectContaining({
        browser_stage: 'fill_form',
        error:
          'PageDriverOperationError: stage=fill_form; Lieju browser operation failed cause=Error: locator.fill timed out; token=[REDACTED]',
  ...
  async function listen(service: LiejuBrowserService): Promise<string> {
...
function fakeService(overrides: Readonly<Record<string, unknown>>): LiejuBrowserService {
  return {
    ...
    ...overrides,
  } as unknown as LiejuBrowserService;
}
