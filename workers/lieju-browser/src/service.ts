...
const LiejuBrowserPublishRequestSchema = LiejuDeliveryInputSchema.extend({
...
export class LiejuBrowserService {
  private readonly locks = new AccountLock();

  public constructor(
    private readonly config: LiejuBrowserConfig,
    private readonly store: PostgresLiejuBrowserStore,
    private readonly driver: LiejuPageDriver,
      ...
      try {
        ...
        if (
          ...
          await this.store.markSession(session, {
            error: { code: error.code, schema_version: 'lieju-browser-error@1' },
            qrExpiresAt: null,
  ...
  public async sessionStatus(accountId: string): Promise<Readonly<Record<string, unknown>>> {
    ...
    return this.locks.run(accountId, async () => {
      ...
      try {
        ...
        console.error('Lieju browser session verification failed', {
        ...
        const attention = await this.store.markSession(current, {
          error: { code, schema_version: 'lieju-browser-error@1' },
          status: 'attention_required',
        });
  ...
  public async publish(
    ...
    const parsed = LiejuBrowserPublishRequestSchema.safeParse(raw);
    if (!parsed.success)
      throw new BrowserGatewayError(400, 'SCHEMA_INVALID', 'Publish payload is invalid');
    if (hashLiejuPayload(parsed.data.payload) !== parsed.data.payload_hash) {
    ...
    const input: BrowserPublishInput = Object.freeze({
      ...
      postingProfile: parsed.data.posting_profile,
    });
  ...
  private async publishLocked(
    ...
    if (session.status === 'attention_required') {
      throw new BrowserGatewayError(
        ...
        'Lieju browser automation is paused for manual attention; login has not been marked expired',
      );
    }
    if (session.status !== 'authenticated') {
      throw new BrowserGatewayError(409, 'AUTH_REQUIRED', 'Lieju browser login is required');
    }
    ...
    if (!authenticated) {
      ...
      void session;
      throw new BrowserGatewayError(409, 'AUTH_REQUIRED', 'Lieju browser login has expired');
    }
  ...
  private async loadImages(
    ...
    const assets = await this.store.loadImageAssets(publication, input.payload.cover_asset_id, []);
    let totalBytes = 0;
    ...
    for (const asset of assets) {
      if (
        ...
        asset.sizeBytes < 1 ||
        asset.sizeBytes > 1_048_576
      ) {
        throw new BrowserGatewayError(
          409,
          'IMAGE_ASSET_INVALID',
          'Lieju image must not exceed 1 MB',
        );
      }
      totalBytes += asset.sizeBytes;
      if (totalBytes > 1_048_576 || images.length > 0) {
        throw new BrowserGatewayError(409, 'IMAGE_ASSET_INVALID', 'Lieju accepts one image only');
      }
      ...
      if (body.byteLength !== asset.sizeBytes || sha256(body) !== asset.contentHash) {
        throw new BrowserGatewayError(
          409,
          'IMAGE_ASSET_INVALID',
          'Lieju image does not match its frozen asset record',
        );
      }
      images.push(
        Object.freeze({
  ...
  private async handlePageDriverOperationFailure(
    ...
    await this.store.markSession(session, {
      error: { code, schema_version: 'lieju-browser-error@1', stage: error.stage },
      status: 'attention_required',
    });
    const updated = await this.store.updatePublication(publication, {
      status: 'manual_required',
    ...
    return new BrowserGatewayError(
      423,
      code,
      'Lieju editor operation failed before submission and requires manual attention',
      error.stage,
      error,
    );
  }
  ...
  private async finishLogin(session: BrowserSession, expiresAt: Date): Promise<void> {
    try {
      ...
      await this.locks.run(session.accountId, async () => {
        ...
        if (!authenticated) {
          ...
          await this.store.markSession(current, {
            error: { code: 'QR_EXPIRED', schema_version: 'lieju-browser-error@1' },
            qrExpiresAt: null,
      ...
      console.error('Lieju browser login verification failed', {
  ...
  private async saveArtifact(
    ...
    const key = `lieju-browser/${publication.tenantId}/${publication.accountId}/${publication.id}/${kind}-${hash}.png`;
