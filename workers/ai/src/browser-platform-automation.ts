...
export function buildBrowserPlatformRewriteInput(
  ...
  const additional =
    platformCode === 'lieju'
      ...
      : platformCode === 'douyin'
        ? '这是抖音自动化图文。保持 platform_meta.content_kind=image_note 以及6-9张封面、正文、总结卡片；封面使用具体钩子，正文单页单重点并覆盖场景、判断标准、步骤和风险边界，删除长段拆页、模板标题与同义重复。description 用160-500字补充条件，不逐页复述卡片。只修复当前质量报告指出的问题；不得伪造热点、排行、亲历、用户评价或无证据资质，AI创作标识由发布器如实设置。'
