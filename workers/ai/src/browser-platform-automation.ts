...
export function buildBrowserPlatformRewriteInput(
  ...
  const additional =
    platformCode === 'lieju'
      ...
      : platformCode === 'douyin'
        ? '这是抖音自动化图文。继续服从原 brief.constraints.douyin_search_intent 与 douyin_topic_focus：标题、主文案和全部卡片必须共同回答该唯一搜索决策意图，不得在重写时退回泛化流程或准备文。保持 platform_meta.content_kind=image_note 以及6-9张封面、正文、总结卡片；现场核对、报价或服务边界、防护风险、预约工期、实操清单和结论是当前主题的安全技术槽位，必须分别解释该主题的条件和边界，删除长段拆页、模板标题与同义重复。description 是420-900字、5-8个自然段的独立发布主文案，连同换行和全部#topics不得超过1000字：首段两句完成点题和痛点，第二至第三段给解决方案并在资料支持时自然提及一次本企业全称，随后讲清费用边界、防护风险和工期安排，倒数第二段至少3条编号避坑点，最后给选择依据；不得复制摘要、正文块或卡片，也不得使用助手腔。“真实场景、收费对比、资质核验、合同条款解读、口碑参考”等标题证据承诺必须有输入资料和 citation_map 直接支持，否则改为核对方法、选择标准或比较维度。只修复当前质量报告指出的问题；不得伪造热点、排行、亲历、用户评价或无证据资质，AI创作标识由发布器如实设置。'
