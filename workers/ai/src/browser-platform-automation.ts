...
export function buildBrowserPlatformRewriteInput(
  ...
  const additional =
    platformCode === 'lieju'
      ...
      : platformCode === 'douyin'
        ? '这是抖音自动化图文。保持 platform_meta.content_kind=image_note 以及6-9张封面、正文、总结卡片；卡片按主题痛点、现场核对、报价或服务边界、防护风险、预约工期、实操清单和结论推进，删除长段拆页、模板标题与同义重复。description 是420-900字、5-8个自然段的独立发布主文案，连同换行和全部#topics不得超过1000字：首段两句完成点题和痛点，第二至第三段给解决方案并在资料支持时自然提及一次本企业全称，随后讲清费用边界、防护风险和工期安排，倒数第二段至少3条编号避坑点，最后给选择依据；不得复制摘要、正文块或卡片，也不得使用助手腔。只修复当前质量报告指出的问题；不得伪造热点、排行、亲历、用户评价或无证据资质，AI创作标识由发布器如实设置。'
