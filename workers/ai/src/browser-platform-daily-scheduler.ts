  ...
  const platformInstruction =
    batch.platformCode === 'lieju'
      ...
      : batch.platformCode === 'douyin'
        ? '输出抖音图文笔记：platform_meta.content_kind 必须是 image_note；生成6-9张图文卡片，顺序为封面、正文、总结。封面用具体问题或收益钩子，正文每页只讲一个判断或动作并控制在24-88字，总结给出可执行清单；整组必须包含适用场景、选择标准、步骤、风险边界，禁止长段拆页和同义重复。同时提供160-500字、不逐页复述卡片的发布文案和3-8个相关话题。不得声明原创、不得伪造热点、排行、亲历、用户评价或无证据资质；发布器会如实勾选 AI 创作标识。'
