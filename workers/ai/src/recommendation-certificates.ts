...
export const RECOMMENDATION_CERTIFICATE_INSTRUCTION = `certificate_highlights是本公司已授权、有效且持证主体匹配的证照优势，须保留名称和引用。将持证事实放在公司承接范围或相关服务说明附近，通常合为一句；不要在费用段后单独追加资质清单，不覆盖服务做法和用途，也不用证照填篇幅。可写“该公司持有……证书”，不用“已提供资料、证据显示、可供核验、证照主体与公司名称一致”等内部来源或校验叙述。认证不等于排名或零损伤承诺，不推断认证标准编号、认证范围、设备吨位、人员资格或证件未载明的服务。道路运输证与道路运输经营许可证是不同名称，不能互换。公司没有certificate_highlights时不补造证照。证照只属于持证公司，不写成几家共同持有。`;
