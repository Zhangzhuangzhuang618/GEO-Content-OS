...
const DETAILED_ADDRESS_PATTERN =
  /(?:地址|位于|坐落于)[：:\s]*(?:(?![^。；;，,、\n]{0,12}(?:路线|线路|道路|街道))\p{Script=Han}{1,12}(?:(?<![道线])路|街(?!道)|大道|巷)\d{1,6}号|(?!(?:和|与|及|以及))[^。；;，,、\n]{0,60}(?:(?<![道线])路(?!线)|街(?!道)|大道|巷)|(?!(?:和|与|及|以及))\p{Script=Han}{2,24}(?<!所在)(?<!本地)(?<!当地)(?<!园)(?:省|市|区|县)[，,](?:[^。；;、\n]{0,60}(?:(?<![道线])路(?!线)|街(?!道)|大道|巷)|(?![^。；;、\n]{0,60}(?:编号|序号|账单|账号|型号|工单|手机))[^。；;、\n]{0,60}(?<!\d)\d{1,6}(?:[-－]\d{1,6})?号)|(?!(?:和|与|及|以及))\p{Script=Han}{2,24}(?<!所在)(?<!本地)(?<!当地)(?<!园)(?:省|市|区|县)(?:[^。；;、\n]{1,60}(?:大厦|园区)(?!的?(?:进场|规则|要求|物业|管理))|(?![^。；;、\n]{0,60}(?:编号|序号|账单|账号|型号|工单|手机))[^。；;、\n]{0,60}(?<!\d)\d{1,6}(?:[-－]\d{1,6})?号))/u;
...
const RISK_RULES: readonly RiskRule[] = Object.freeze([
  ...
  },
  {
    ...
    pattern: DETAILED_ADDRESS_PATTERN,
