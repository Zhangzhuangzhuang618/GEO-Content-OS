...
| douyin | 抖音 | 多页图文/旧口播脚本包 | 图文卡片与托管浏览器发布；旧脚本包兼容导出 |
...
HOTFIX-20260822-05 修复 Cloudflare `@cf/black-forest-labs/flux-1-schnell` 当前 REST 输入契约拒绝
`seed` 字段而导致所有正文图生成请求返回 HTTP 400 的问题：生成请求只发送当前模型接受的 `prompt`
和 `steps`，视觉质检、模板降级、对象存储和文章质量门槛不变。历史 `fallback` 运行不自动重跑；部署后
新建的配图任务按新契约调用。

### 抖音图文质量增强（ADR-0074 / T159）

新生成的抖音图文收敛为 6–9 张短卡，按问题钩子、适用场景、判断标准、操作步骤、风险边界和
总结形成完整叙事。HOTFIX-20260827-02 将发布说明明确为独立主文案：420–900 字、5–8 个自然段，
首段用两句话完成点题与现实痛点，正文依次覆盖解决方案、报价或服务边界、防护与责任、预约或工期，
并包含至少 3 条编号实操建议和行动结论；有已发布企业法定名称时，至少在第二或第三段自然出现 1 次，
最多 2 次，且不得超出企业资料证据。服务端对标题、单卡长度、通用模板词、跨页近重复、主文案结构、
卡片或正文复制和助手式模板表达执行确定性质量门禁；`fast` 生成也不得绕过抖音专用门禁，不达标仍
走现有质量引导重写，不伪造通过结论。旧内容、旧质量报告和旧发布任务不自动重新评估。

HOTFIX-20260828-01 将同一抖音图文叙事策略接入发布前确定性质检，人工修改和其他旁路也必须满足标题、
主文案结构、话题、卡片及当前企业名称位置与频次要求。创作者中心实际填写的 `description`、换行和
全部 `#topics` 合计不得超过 1000 字；生成、质检、Render 与 Browser Worker 共用同一拼接和长度规则。
旧 `script_package` 继续兼容，旧内容和旧报告不自动重评；已排期旧任务仅在实际输入超过技术上限时
于最终发布前失败关闭。无数据库、公开 API、生产配置或冻结质量阈值变化。

HOTFIX-20260828-02 将抖音专用 Prompt 放到通用示例之后并在抖音生成时移除冲突示例，输出契约在模型
返回阶段直接约束 `image_note` 的主文案、话题和 6–9 张卡片结构。抖音首稿未通过既有确定性叙事门禁时，
最多执行 3 次含首稿的质量报告驱动生成，仍不合格则失败关闭；不得由服务端改写内容或伪造通过结论。
托管浏览器比较编辑器回填内容时忽略平台插入的零宽分隔符，其他可见文本仍须逐字一致。旧内容、旧报告
和旧任务不自动重评，无数据库、公开 API、生产配置或冻结质量阈值变化。

### 抖音登录二次验证（ADR-0076 / T161）

扫码授权后创作者中心要求短信或原设备扫码时，抖音 Worker 保留当前账号的独立 Profile 和挑战页面，
PUB-01 通过既有登录或重新认证端点显式发送短信、提交一次性验证码或取得原设备二维码。状态轮询先检查
当前挑战，不再打开编辑页破坏验证；只有明确出现登录证据后才加密保存 storage state。短信只能由用户
点击发送，验证码不落库、不进对象存储或日志。失败诊断只保存隐藏全部文字与背景图并遮盖输入框、图片、
二维码、头像和账号区域后的私有截图，以及去除查询参数的页面位置、挑战类型、可用方式、内容哈希和白名单页面签名。滑块、拼图
和未知交互仍失败关闭，不识别或绕过平台风控。本功能不新增公开端点、数据库表或迁移。

HOTFIX-20260828-04 修正 PUB-01 把高度脱敏的审计截图大幅展示为操作画面、导致页面看似损坏的问题：
普通用户界面只显示挑战类型、页面位置、可用验证方式和诊断证据已保存提示，不再渲染脱敏截图；短信
验证码、原设备二维码、私有诊断截图采集与存储、账号隔离和验证码失败关闭规则均不变。无数据库迁移、
公开 API 或生产配置变更。

配图在 ADR-0073 确定性文字卡基础上升级为“生成背景 + 服务端叠字”：DeepSeek 仅从当前合格卡片
中选择 3–5 个需要插画的场景，Cloudflare 生成 3:4 匿名编辑插画并执行既有视觉门禁；中文标题、
正文、页码和 AI 标识始终由服务端绘制。单页生成或视觉质检失败时只回退该页的编辑模板，完整记录
降级原因，不改变文章质量结论。旧抖音内容、旧质量报告、旧素材和旧发布任务不自动重评或重做。

HOTFIX-20260822-06 根据 ADR-0070 按列举网官方确认口径收敛平台自加门禁：正文允许相关网址、
裸域名和官方核验链接，`权威`、`国家级` 不再作为无条件字面禁词；本企业品牌事实和由当前企业授权
证据直接支持的资质声明不是平台默认禁区。电话、微信、QQ、极限词、排名、虚假或无证据声明、
跨企业冒用及全部冻结质量分数继续阻断。生成、质检、重写、确定性扫描和 Render 使用同一边界；
无数据库迁移或公开 API 变更，旧内容、旧报告、旧批次和旧尝试不自动重评。

### 企业证照资料（ADR-0053 / T155）

“导入资料”支持 PNG、JPEG、WebP 企业证照图片。证照名称、编号、持证主体和发证机关由用户人工确认，可选填写 HTTPS 官方核验链接并沿用资料有效期；生产环境不依赖 OCR，也不从图片像素伪造事实。原图保持私有，只有当前内容实际引用、明确授权公开、未失效且持证主体精确匹配当前工作区已发布企业法定名称时，才生成移除文件元数据的随文副本。官网、百家号和搜狐号可使用该副本，官网标注为可核验证照而非 AI 图片；列举网首版不附带。证照媒体失败无图降级，不改变文章冻结质量门槛、旧内容或旧质量报告。

HOTFIX-20260819-06 将证照扫描件从普通生成配图门槛中分离：证照输入允许最长边 8192 像素、总像素不超过 5000 万且文件不超过 10 MB，原图继续私有保存；随文发布仍重新编码为完整画面、不裁切、移除元数据的 1200×800 JPEG。普通生成图片的 4096 像素门槛、冻结质量门槛、旧资料、旧内容和旧质量报告均不变，无数据库迁移。

HOTFIX-20260819-07 将 HOTFIX-20260819-06 的高分辨率输入门槛扩展到所有导入资料图片，覆盖道路运输许可证、信用证书及后续其他图片资料；普通图片保留 25 MiB 上限，证照结构化字段、10 MB 上限和受控随文展示规则保持不变，普通生成配图不放宽。无数据库迁移，不自动重新处理旧资料。

### 资料有效期修正（ADR-0075 / T160）

具备资料管理权限的租户成员可在资料详情修改有效期。更新只写入当前资料的 `effective_from`、
`effective_to` 和更新时间，使用当前 `updated_at` 做乐观锁，并将修改前后值及原因写入审计；不重新
解析资料、不改内容哈希、原文片段、事实、历史引用或已生成内容。普通资料允许一端或两端留空；
保险证明继续要求完整保障期间。新的检索和生文立即按修正后的有效期过滤，历史结果不追溯重算。
...
HOTFIX-20260817-07 修正所有平台共用的质检语义阻断：共享校验同时识别 `title_max_length` 与列举网使用的 `title_max_characters`，拒绝“标题未超限却返回标题长度 BLOCK”等自相矛盾结论；语义修复提示只允许对输入中真实存在的高风险无支持或冲突事实生成精确 claim 问题。一次进程内语义修复后仍返回 `SKILL_OUTPUT_INVALID` 时立即失败收尾并进入人工处理，不再重复队列级模型调用；其他错误保持既有队列策略。冻结门槛、旧内容和旧质量报告保持不变，服务端不伪造通过结论。

HOTFIX-20260817-08 根据列举网 attempt 2 的 0/30 生产复盘继续对齐生成与质检语义：补齐 `.title.max_characters` 标题规则变体；把 `contact_in_content_forbidden` 明确为具体电话、网址、微信 ID 或 QQ ID，不再把“通过页面联系方式咨询”误判为联系方式；“电话公司”等行业泛称不再视为第三方专有名称。无高风险事实时同时移除 few-shot 示例对中的对应高风险事实与 BLOCK，并在首次质检注入当前标题字数、允许 claim 和联系方式边界。列举网首稿、日批简报及重写统一使用 5–30 字问题/方法型标题，以满足冻结的问题覆盖门槛。旧候选和旧报告不重新评估，部署后需显式发起新尝试。

HOTFIX-20260817-09 修复搜狐号与列举网人工质检无法回到全链路自动化的问题：用户重新质检没有发布任务的人工内容时，同步恢复浏览器平台运行、批次项及当日批次；发布管理页可一键批量重新质检全部内容类失败项，发布失败项仍必须处理原发布任务。不可验证的模型品牌问题继续按既有规则触发一次完整模型重判，重判仍无效时失败关闭；服务端不删除模型问题、不改写模型分数或决策。旧内容、旧报告及旧发布记录继续保留。

HOTFIX-20260817-10 修复品牌质检语义重判只有通用错误、无法针对首次误判纠正的问题：品牌断言按分类、严重级别、位置、准确名称、自家公司或行业泛称、名称位置不一致输出稳定的脱敏原因码，并把原因码传给既有的一次完整模型重判。不得记录模型原始消息或正文，不增加模型调用次数；重判仍无效时继续失败关闭，服务端不删除模型问题、不改写质量分数或决策。

HOTFIX-20260817-11 修复列举网官方 API 发布被跨平台媒体字段阻断的问题：列举网渲染只注入其 schema 支持的封面素材 ID，不再携带搜狐号/百家号专用的正文素材 ID；官方 API 凭据不再混入浏览器网关账号作用域字段。没有持久公网媒体地址时，列举网自动化在质量通过后直接无图排期，不创建无效配图任务；搜狐号、百家号和官网配图保持不变。旧发布任务和旧媒体记录不自动改写，部署后需显式重试原失败任务。

HOTFIX-20260818-01 修复列举网禁词校验将“第一方”证据属性表达误判为排名宣传的问题：“第一”不再匹配紧随“方”的表达，“行业第一”等排名用语及其他高风险宣传词仍然阻断。不改变冻结质量门禁，不自动重新评估旧内容或旧报告，部署后需显式重试原失败发布任务。

HOTFIX-20260818-02 为列举网官方 API 未知响应补充脱敏诊断：只保留 HTTP 状态、规范化 Content-Type、字节数、响应类型、SHA-256、白名单 JSON 字段名和登录/验证码/跳转信号，写入发布尝试、任务错误和列举网提交记录。不记录原始响应、JSON 值、API Key、文章或联系信息；不改变成功判定、未知态、人工对账和禁止盲目重投规则。旧记录不回填。

HOTFIX-20260818-03 对齐列举网当前“生活服务 / 搬家”发布页的现行必填字段：官方 API 在保留 `postdb[fid]=73` 的同时发送二级类别 `postdb[leibie]=4`（搬家搬场）和用户配置的 `postdb[dizhi]` 发布地址，并按逗号边界清洗成功文本中的公开 URL。新账号必须填写地址；旧官方 API 账号可只补地址并保留原加密 Key、联系人、电话和区域。2026-08-18 受控真实提交仅执行一次，返回内容 ID `105541615`，公开页标题核验一致。无数据库迁移，不改变冻结质量门禁、未知态或禁止盲目重投规则；旧任务和旧记录不自动处理。

HOTFIX-20260818-04 修正浏览器平台未知发布结果的过期处置入口：详情接口与处置事务共用任务状态、内容变体状态和当前内容版本判断；内容已进入重新质检、其他流程或产生新版本时，PUB-03 只说明旧任务已冻结，不再展示必然返回 409 的“确认未发布并重试”“确认未发布并结束任务”或“确认已经发布”。仍处于 `publish_failed` 且指向当前内容版本的百家号、搜狐号和列举网任务继续使用原人工核实闭环；不会改成绕过提交预留和防重复保护的普通重试。无数据库迁移，不改写旧任务、旧尝试或旧质量报告。

HOTFIX-20260818-05 根据 ADR-0048 收窄列举网发布前“第一”禁词匹配：放行“第一步、第一阶段、第一时间、第一年”等普通顺序或时间表达，继续阻断“行业第一、排名第一、第一品牌、自称第一”等明确排名宣传及既有高风险词。任务仍在 Publisher 调用官方 API 前执行确定性校验；不降低冻结质量门槛，不改写模型结论、旧内容、旧报告或旧尝试，部署后需显式普通重试原发布任务。

HOTFIX-20260818-06 修复浏览器平台未知发布结果经人工确认未发布后仍返回 409 的状态映射错误：最新尝试为 `unknown` 或错误码为 `MANUAL_REQUIRED` 时，自动化运行按实际落盘的 `manual_required` 精确恢复；列举网官方 API 已提交但仍 processing 的成功尝试继续按 `publish_failed` 恢复。普通重试仍受未知态防重复保护，最多 3 次尝试、历史尝试追加只读、旧内容和旧质量报告均不变。无数据库迁移，部署后需先在平台后台确认未发布，再使用原任务的“确认未发布并重试”。

HOTFIX-20260818-07 将列举网浏览器发布后的“信息审核中！”确认页识别为平台已接受提交：Worker 命中后立即返回 `processing`，不再强制等待标题先出现在会员中心；平台未提供内容 ID 时保留空远端 ID，并使用本地发布记录继续对账。只有公开页可访问且标题匹配才更新为 `published`，失败、验证码、登录失效和多条匹配仍按原规则处理。不修改冻结质量门槛、旧内容、旧报告或历史发布尝试，无数据库迁移。

HOTFIX-20260819-01 根据 ADR-0050 修正上一条中“已接受但继续 processing”的错误产品语义：列举网浏览器出现精确“信息审核中！”确认页，或官方 API 返回通过既有校验的明确成功响应时，均直接完成 GEO 发布任务。这里的 `published` 表示列举网已接受并进入平台审核，不表示审核通过或已公开展示；登录、验证码、拒绝、HTTP 5xx、网络未知和无法识别响应仍不得判成功。浏览器未取得远端内容 ID 时持久化本地发布记录 ID，保证 Publisher 终态核对一致。旧任务不自动重投，不修改冻结质量门槛、旧内容或旧报告，无数据库迁移。

HOTFIX-20260819-02 修正百家号内容列表把同一远端文章的重复 DOM 节点误判为多条匹配的问题：取得严格发布回执后按回执文章 ID 精确核对，后续对账也优先使用已保存文章 ID；列表候选先按远端文章 ID 去重，只有不同文章 ID 的多个标题候选才进入人工处理。多匹配错误只记录原始节点数和去重后数量，不记录标题、正文、Cookie 或凭证。未知态禁止盲目重投、验证码和登录安全暂停、冻结质量门槛及旧任务状态均不变，无数据库迁移。

HOTFIX-20260819-03 修复列举网账号加密发布资料无法纠正导致同一 API Key 下部分账号持续失败的问题：账号编辑页可局部更新区域、地址、联系人、电话、QQ 和微信号，并通过独立选项显式清空 QQ 或微信号；空白输入仍表示不修改，凭据和联系方式继续不回显。创建、编辑和 Adapter 最终配置统一执行列举网当前电话、QQ、微信号格式校验；旧错误资料在发出外部请求前明确失败，不占用未知态人工对账；“请正确填写微信号”等现行字段提示记为明确拒绝，不再误报状态未知、登录或 API 权限问题。旧账号、旧任务、旧内容和旧质量报告不自动改写或重投，未知旧任务仍须先人工确认未发布；无数据库迁移，不改变冻结质量门槛和未知态防重复规则。

HOTFIX-20260819-04 在列举网官方 API 账号的新建和编辑界面明确标注平台必填资料：API Key、广州区域、发布地址、联系人和联系电话标为“列举网必填”，QQ 与微信号标为可选。编辑页同时说明必填资料只需已存在于账号配置，留空表示沿用加密保存值，无需重复输入。仅调整 Web 表单说明，不改变字段校验、凭据保存、发布请求、质量门槛、旧账号或历史任务；无数据库迁移。

HOTFIX-20260819-05 根据 ADR-0052 修复 Publisher 官网发布校验仍硬编码志远企业名称导致跨租户误拦截的问题：领取任务时按 tenant/workspace 读取当前工作区已发布品牌档案，提取完整法定企业名称并显式传给官网 Render Adapter；当前企业名称允许出现，其他企业名称继续阻断，缺少名称声明时保持失败关闭，不做地区或简称模糊放行。官网 Render 输入契约升级到 `official-site-render-rules@1.2.0`。生产证据中的失败任务实际为众人搬家官网任务，尚未进入列举网外部请求。无数据库迁移，不降低冻结质量门槛，不改写或重投旧任务、旧内容和旧质量报告；部署后应核对品牌档案与正文法定名称一致，再显式普通重试。

HOTFIX-20260820-03 根据 ADR-0056 将列举网发布层禁止宣传词前移到生文、生成后质量检查、AI Worker 确定性质检和质量报告引导重写，并由 Contracts 统一提供字面匹配。发布层继续阻断“百分百”等既有高风险词；否定、引用、风险提示和举例也必须删除原词后中性改写，“第一步、第一阶段、第一方”等普通表达继续放行。该修复不由服务端静默改文或伪造质量结论，不降低冻结门槛，无数据库迁移和公开 API 变更。旧内容、旧报告、旧任务和旧批次不会自动变化；含禁词的旧任务必须显式重新质检并完成重写，或发起新的日批尝试，直接重试发布仍会失败。

HOTFIX-20260829-01 对齐浏览器发布的 300 秒上游等待、600 秒 BullMQ 锁与数据库租约，Publisher 默认全局并发收敛为 1 并在执行期间续租；四个浏览器 Worker 只在不可逆提交前进入 `submitting`，提交前明确崩溃时释放账号 Context 并单次重试，点击后异常仍禁止盲目重投。BullMQ 最终失败为非终态数据库任务幂等补建最多 3 次 Outbox 恢复事件。企业证照复合声明可由持证主体和证照名称逐项精确的授权引用共同支持；质量检查只移除报告位置不存在所点名第三方企业的误报。Content Writer 同模型两次非法结构后可调用既有质量模型低温兜底一次，日批淘汰项保留实际生成错误，门禁重写提示包含分数差距和动作。无数据库迁移、公开 API 或冻结阈值变化，不自动重跑旧记录；ADR-0023 的候选上限内持续补位规则不增加硬熔断。

HOTFIX-20260829-02 补齐抖音短信二次验证的接收号码提示与重发入口：Worker 只从可见页面提取并白名单返回形如 `138****5678` 的脱敏手机号，完整手机号继续由契约拒绝；验证码输入阶段识别“重新发送/重新获取”等控件并返回实时可用状态，Web 始终展示重发按钮并在抖音倒计时结束后自动启用。短信发送仍必须由用户明确点击，验证码不落库、不写日志，不绕过抖音安全验证。无数据库迁移，不改变发布质量门槛、登录成功判定或历史记录。

HOTFIX-20260829-03 修正抖音短信方式选择后过早返回、重发控件被遮罩仍误报可用及前端遗漏映射后 409 的问题，并在登录二维码过期后停止无效轮询。短信发送和重发继续只由用户明确触发，遮罩或交互式验证返回人工处理，不绕过平台安全策略。无数据库迁移、公开 API、生产配置或发布质量门槛变化。

HOTFIX-20260829-04 区分抖音身份验证选择层中的“接收短信验证码”和语义不同的“发送短信验证”：Worker 只操作前者，并把控件查找限制在当前前景验证层。验证码输入框只有在可见、可编辑且未被遮罩时才视为可输入；页面签名、私有诊断和日志增加前景层及各验证选项的布尔证据、脱敏截图 SHA-256 和私有对象位置。截图仍隐藏全部文字和背景并遮盖身份与输入内容；新增日志不记录验证码、完整或脱敏手机号、账号正文或原始 DOM，既有脱敏手机号白名单响应不变。无数据库迁移、公开 API、生产配置、登录成功判定或发布质量门槛变化。

HOTFIX-20260829-05 修正抖音身份验证浮层没有标准 `role=dialog` 时的短信方式选择顺序：Worker 从“身份验证/短信验证码”等可见锚点向上识别同时包含至少两项安全验证证据的最小前景容器；只要该容器存在“接收短信验证码”，就先精确选择该方式，再判断验证码输入框和发送控件，不再被浮层下方可见但不可操作的旧输入框提前拦截。“发送短信验证”和刷脸方式仍不会被自动操作；输入框被其他安全层遮挡且没有可用接收方式时继续失败关闭。无数据库迁移、公开 API、生产配置、登录成功判定或发布质量门槛变化。

HOTFIX-20260829-06 修正抖音短信已发送后 GEO 页面不展示验证码提交区的问题：Worker 不再仅以输入框中心点能否接收鼠标点击判定整个短信步骤。输入框可见、启用、可编辑，且同一前景验证层的提交控件可操作时，即上报 `has_code_input=true`，使 Web 显示验证码输入和提交功能。提交前仍单独校验提交控件未被安全层遮挡，不使用强制点击，全屏验证遮罩继续返回人工处理。无数据库迁移、公开 API、生产配置、登录成功判定或发布质量门槛变化。

HOTFIX-20260830-01 修正抖音短信验证码输入区因动态验证层时序而偶发不展示的问题：Worker 将“输入框可见、启用且可编辑”与“当前可安全点击并提交”拆分，方式选择后在有界时间内等待短信页面稳定；Web 在整个 `attention_required` 阶段持续复核，不再因一次 `has_code_input=false` 停止轮询。提交验证码前仍重新校验输入框和提交控件的命中目标，遮罩或图形验证码继续失败关闭。私有诊断升级为 `douyin-login-verification-diagnostic@3`，记录输入框及提交控件的可见、启用、编辑和命中目标布尔证据，不记录原始 DOM、控件正文、验证码或手机号；截图优先裁切前景验证层，页面文字与媒体继续隐藏，仅叠加无身份信息的安全状态摘要，移除会误遮整页的宽泛 class 掩码。无数据库迁移、公开 API 形状、生产配置、登录成功判定或发布质量门槛变化。

HOTFIX-20260830-02 修正抖音扫码后停留在身份验证方式选择页时仍突出显示“生成登录二维码”，导致操作人员误把“尚未选择短信方式”理解为验证码输入框故障并反复重置登录的问题。Web 在 `attention_required` 阶段隐藏普通二维码入口，明确提示扫码已经完成，并把重新扫码收敛为需确认的“放弃当前验证”操作；Worker 将预期的安全挑战记录为“需要额外验证”，并仅记录已接收的验证动作类型，便于区分扫码重试与短信发送。日志不记录验证码、手机号、账号正文或页面正文；短信发送仍必须由用户明确触发。无数据库迁移、公开 API、生产配置、登录成功判定或发布质量门槛变化。

HOTFIX-20260830-03 修正抖音登录二维码已经生成后 Web 仍允许直接再次生成、以及已被新尝试替代的异步二维码流程继续打印二次验证或失败日志的问题。`qr_ready` 阶段隐藏普通生成入口，替换二维码必须通过独立确认；Worker 只在核对会话版本、状态和过期时间仍属于当前尝试后记录结果并保存诊断，陈旧尝试静默退出。生产证据未出现任何二次验证动作到达 Worker 的日志，因此本修复不推测或修改短信页面选择器；后续只有在同一时间窗取得 API 请求结果和 Worker 动作日志后才判断短信链路。无数据库迁移、公开 API、生产配置、登录成功判定或发布质量门槛变化。

HOTFIX-20260830-04 修正抖音登录态每 3 秒无条件并发轮询导致慢请求在 Worker 账号锁前持续排队、最终被 Next.js 默认 30 秒代理超时以 `ECONNRESET` 中断的问题。Web 改为上一轮完成后再等待 3 秒发起下一轮，失败时明确展示 HTTP 状态并继续顺序重试；同源 API 代理超时调整为 70 秒，略高于 API 到抖音 Worker 的默认 65 秒上限，保证由 API 返回结构化结果。无数据库迁移、公开 API、生产环境变量、抖音页面识别、登录成功判定或发布质量门槛变化。

HOTFIX-20260830-05 修正抖音二次验证页高负载时，登录与状态查询同时等待同一账号浏览器锁并超过网关时限的问题。状态查询在同账号操作进行中直接返回已保存的安全诊断，不再排队；空闲时的二次验证复核只读取页面控件状态，不再每 3 秒重新生成脱敏截图。明确重新扫码时先关闭旧浏览器上下文，headless Chromium 在无 GPU 的容器内禁用 GPU 加速，避免沿用异常 GPU 进程。诊断截图仍在首次挑战、验证动作结果和失败时生成并写入私有对象存储。无数据库迁移、公开 API 形状、生产环境变量、验证方式选择、登录成功判定或发布质量门槛变化。

HOTFIX-20260830-06 修正抖音短信验证码提交后验证层已经关闭，但首页普通验证码输入元素仍被全页面选择器误判为活动验证步骤，导致登录成功检查被短路的问题。验证码输入只在已确认的前景验证容器内识别；验证层消失后继续检查登录结果。headless Chromium 同时移除 Playwright 默认注入的 SwiftShader 参数并禁用软件光栅化，修复生产软件 GPU 子进程持续高 CPU 对页面检查和代理时限的放大；无确认验证容器时的诊断截图收敛到当前视口。现有证据不证明本次验证码是否正确或已被平台接受；系统仍以抖音真实登录态为准，不自动选择验证方式、发送短信或绕过安全验证。无数据库迁移、公开 API 形状、生产环境变量或发布质量门槛变化。

HOTFIX-20260830-07 修正抖音 Worker 重启后数据库仍保留 `attention_required`，但只存在于旧进程内存中的二次验证页面已经失效，状态复核仍反复新建浏览器并返回旧验证入口的问题。页面不存在或关闭时检查明确返回 `AUTH_REQUIRED`，状态服务沿用既有失败关闭路径切换为 `reauth`，不再导航失效会话或造成周期性代理超时；活动页面仍存在时继续执行 HOTFIX-20260830-06 的验证层与登录态判定。未完成验证码不会恢复或持久化，操作人员必须重新扫码。无数据库迁移、公开 API 形状、生产环境变量或发布质量门槛变化。

HOTFIX-20260830-08 修正生产将 `CONTENT_MODEL_QUALITY_KEY` 与 `QUALITY_CHECKER_MODEL_KEY` 错配为 Flash，导致抖音日批的 Quality 生成退化为 Flash、同模型结构化重试持续失败的问题。DeepSeek 驱动启动时拒绝内容质量模型或 Quality Checker 与 Fast/Balanced 使用相同模型键，并在启动日志中只记录非敏感模型路由；生产恢复 ADR-0011 规定的 Fast/Balanced=Flash、Quality/Checker=Pro。2026-08-30 两个失败批次的 18 个候选中，11 个为结构化输出失败、7 个为既有质量门禁淘汰，Outbox 与 Worker 均健康。历史批次不自动重跑，部署后必须显式重新发起；无数据库迁移、公开 API 或冻结质量阈值变化。

ADR-0077 / HOTFIX-20260830-09 将服务端已绑定品牌资料、平台规则和引用的抖音日批改为受限结构 Flash-first：模型只填写标题、叙事段落和固定 7 张语义卡片的文字，服务端持有卡片键、顺序、类型、内容块和冻结封装；该路径从首稿开始不注册工具。首稿继续按 quality 确定性门禁验收，失败时只由 Flash 定点修复一次，结构无效或仍不合格才由 Pro 按相同浅层 Schema 兜底；独立 Quality Checker 和质量报告驱动重写继续使用 Pro。服务端不改写模型事实、不降低抖音编辑、企业名称、资质证据或发布门槛；无数据库迁移、公开 API、新环境变量或其他平台路由变化。

HOTFIX-20260830-10 修正受限草稿把文字最小长度放进浅层 JSON Schema，导致可修复短字段先以结构错误失败并直接升级 Pro 的问题。浅层 Schema 只保留字段、类型、数组形状和最大长度；原最小长度作为带准确目标 ID 的服务端确定性问题先交给 Flash 定点修复，并在首稿提示中明确全部生产长度范围。修复后仍须通过原最小长度、ADR-0074 编辑门禁、企业名称、资质证据和冻结 Content Writer Schema，仍不合格才调用 Pro。质量报告 `decision=pass` 不代表浏览器自动化门禁通过；自动化继续以 `automation_gate_json.passed` 及全部冻结分项阈值决定排期或重写。无数据库迁移、公开 API、新环境变量或门禁变化。
...
列举网独立内容允许真实、具体、有边界地介绍本企业服务范围、流程、可核验能力和适用场景，并可自然提示用户通过页面联系方式咨询或提交需求。正文允许相关网址、裸域名和官方核验链接；本企业品牌事实及由当前企业授权证据直接支持的资质声明不是平台默认禁区。正文仍禁止电话、微信、QQ、极限词、排名、竞品贬损、虚假价格、无证据资质、虚构案例、客户评价和结果保证；跨企业冒用、事实冲突与冻结质量门槛不变，联系人与电话只由加密账号配置在发布表单中填写。
...
| `source_documents` | 原始可信资料、企业证照与保险证明私有原件 |
...
ADR-0055 / T156 新增“保险证明”资料类型：PDF 原件保持私有，仅人工确认的投保主体、承保机构、
保险类型、保障期间和参保人数生成确定性脱敏摘要并参与分块、索引、检索与生文。保险单号、人员
名单、证件号、电话、邮箱、银行卡及 PDF 正文不得进入元数据、分块、向量、RAG 或模型上下文；
资料标题固定为“企业保险证明”，生产首版不对扫描件运行 OCR，也不生成公开媒体副本。无效期外
资料继续由既有有效期过滤排除，旧资料、旧内容、旧质量报告和旧发布任务不会自动重建或重新生成。

HOTFIX-20260820-04 修复资料详情和单个解析任务响应把数据库内部 `retryable` 字段原样透传、导致
严格前端契约拒绝整份 HTTP 200 响应的问题。API 现在只投影公开 `job-error@1` 契约允许的 code、
message 和 schema_version；内部重试判定与历史错误 JSON 保持不变。包含旧失败解析任务和新成功
任务的企业保险证明仍可正常加载。无数据库迁移、公开契约或 Web 变更，历史资料无需重新解析。

HOTFIX-20260820-05 根据 ADR-0057 修复搜狐号与列举网日批中“证明已上传但未进入文章证据链”的
资质阻断，并收敛本轮生产暴露的质检失败。新候选在既有最多 5 个片段内为当前企业已授权、有效且
主体精确匹配的结构化证照预留最多 3 个位置；资质声明必须通过 citation_map 逐项绑定当前企业证照，
证照 source ID 随候选冻结，质检时仍会重新核验当前范围、授权、公开确认和有效期；缺证、错主体、
未授权、无声明映射或仅伪造证照文本时仍然 BLOCK。质量重写改为复用原始不可变 Writer 输入且不再
调用策略/平台规则工具；Quality Checker 的一次语义恢复使用独立修复任务并放行“某银行”等明确
匿名称谓，同时拒绝把普通正文短语误报为企业名。PUB-01
不再向用户展示内部错误码、英文模型消息和校验 JSON。无数据库迁移或公开 API 变更，不降低冻结
门槛；旧候选原输入没有证照时必须显式发起新的日批尝试，旧内容和旧质量报告不会自动重评。

HOTFIX-20260820-06 根据 ADR-0058 修复 Quality Checker 在一次模型语义修复后仍把当前企业名称或
明确匿名称谓误报为其他企业、继而以 `SKILL_OUTPUT_INVALID` 终止整条任务的问题。首次输出仍完整
校验并只允许一次独立模型修复；第二次输出仅删除服务端能够精确判定为
`only_allowed_owner_or_generic_name_is_quoted` 的品牌问题，再按剩余模型问题和冻结 warning 门槛
重算 decision，并继续执行原有确定性风险扫描。真实第三方企业和具名平台仍然 BLOCK；分类、位置、
严重度或引用目标不能安全验证的品牌问题，以及事实、证据、标题、联系方式和 citation 硬规则均不
采用此恢复。无数据库迁移、公开 API、前端契约或额外模型调用；历史内容和质量报告不会自动重评。

HOTFIX-20260820-07 根据 ADR-0059 为两类连续模型不合规增加条件式最终修复。Quality Checker 首次
语义修复后若第二次失败的脱敏拒绝全部属于 `brand.other_company_name`，最多再进行一次完整模型修复；
非品牌错误仍保持一次上限，最终结果仍执行全部冻结断言，服务端不删除问题或伪造分数和结论。搜狐号
与列举网质量重写在结构已合格且剩余问题仅为证照 citation_map 绑定及可能的内容未变化时，最多再做
一次定向重写：只使用原候选冻结证照，有证据则逐项绑定，缺证或不能完整证明则由模型删除声明；仍不
合格继续人工处理。无数据库迁移、公开 API、前端或状态机变更，旧候选不加载后来上传的资料且不会
自动恢复。

HOTFIX-20260820-08 根据 ADR-0060 为发布任务详情补充任务实际绑定的不可变全文快照，并为已排期
任务增加安全人工修改路径。只有 owner/admin 可先取消尚未执行的任务再进入编辑器；保存产生新内容
版本并携带源任务重新质检，自动化来源通过后创建绑定新版本的新任务，人工来源通过后由用户按现有
审核流程重新排期。
不通过时保留人工版本且不触发 AI 重写。旧任务、尝试、报告和版本保持不变；扩展既有发布详情响应与
质检请求契约并同步 OpenAPI，无数据库迁移，不降低质量门槛，也不自动处理既有任务。

HOTFIX-20260820-09 根据 ADR-0061 将浏览器平台证照最终修复从整篇重写改为最多两轮的精确目标
替换：模型只返回含不受支持证照声明字段的替换文本，服务端不改动无关字段、顺序或锁定区块，不接受
缺失、未知、重复、空白、未变化或仍含问题声明的返回。Quality Checker 的既有语义修复若重复返回
列举网联系方式问题，且准确位置确定没有电话、网址、微信 ID 或 QQ ID，仅移除该条不可验证误报，
保留模型分数并继续全部确定性门禁。无数据库迁移、公开 API 或前端变更；旧候选和旧报告不会自动
重评。

HOTFIX-20260820-10 根据 ADR-0062 修复列举网重新质检仍被证照核验建议、错误事实位置和非企业名
称谓阻断，以及证照与推广词并存时定向重写无法收敛的问题。证照扫描只放行“核验是否具备”“选择时
应确认”等非断言式建议，当前企业实际持有、有效或齐全的断言仍必须由冻结证照及 citation_map 逐项
支持；浏览器平台最终修复可在同一轮精确替换中同时删除无证据证照声明和列举网禁止推广词。Quality
Checker 只在既有模型语义修复后，删除位置确定不具备高风险事实资格或引文确定不是可识别企业名称的
对应误报，模型分数保持不变并继续全部确定性门禁。拒绝定向替换时只记录有限脱敏原因，不持久化模型
原始输出。无数据库迁移、公开 API 或前端变更；旧候选仍使用首次生成时冻结的资料，旧内容和旧质量
报告不会自动重评。

HOTFIX-20260821-01 根据 ADR-0063 修复共享日批证据检索请求 ID 超过 Embedding 与 Citation Search
80 字符上限、导致官网、百家号独立生文、搜狐号和列举网无法创建候选的问题。主题与证照检索继续
使用完整输入计算 SHA-256，但嵌入和搜索请求 ID 只携带前 32 位哈希，并保留用途与阶段前缀；四类
请求 ID 均保持确定性、彼此区分且不超过适配器边界。无数据库迁移、公开 API 或质量门槛变化；错误
...
HOTFIX-20260822-01 根据 ADR-0068 统一列举网生文、质量检查、确定性质检和发布前校验对具体联系
方式与网址的识别，并补齐无协议裸域名。整篇质量重写仍保留具体值时，可在既有最多两轮定向修复中
只替换命中字段；证照核验说明保留官方渠道名称和核验动作，但删除域名，任何仍含网址、电话、微信
或 QQ 具体值的替换均拒绝。锁定区块、冻结质量门槛和模型结论边界不变；无数据库迁移、公开 API、
状态机或 Web 变更，旧内容、旧报告、旧批次和旧任务不会自动重评。

HOTFIX-20260822-02 修复百家号托管浏览器长期复用崩溃页面、导致一次成功后后续发布持续失败，以及
“检查并恢复”只验证首页登录态便解除安全暂停的假恢复。每次登录核验、发布或对账完成后均回收账号
浏览器上下文，下次操作从加密 storage state 与临时 Profile 建立干净会话；解除 `attention_required` 时除登录有效外
...
HOTFIX-20260822-04 根据 ADR-0069 将百家号取得成功回执后的前 3 轮状态核验调整为约第 1、3、5
分钟执行，第 4 轮起仍每 5 分钟一次，最大 12 轮不变。搜狐号和列举网继续保持原 5 分钟节奏；每轮
仍经 Outbox 幂等投递并读取真实内容列表，预览 URL 或提交回执不视为已发布。无数据库迁移、公开 API、
状态机、质量门槛或 Web 变更；已排队的旧对账事件不追溯改期。
...
| M16 | T155 | 企业证照图片资料、结构化核验字段与受控随文展示 |
| M17 | T156 | 保险证明 PDF 私有原件、人工确认字段与脱敏检索摘要 |
| M18 | T157 | 问天项目绑定、一次性进入票据和问题集同步 |
| M19 | T158 | 抖音图文生成、托管浏览器发布与真实链路验收 |
| M20 | T159 | 抖音短卡文案门禁、场景化插画背景与服务端安全叠字 |
| M21 | T160 | 资料有效期修正、乐观锁、审计和详情页编辑 |
| M22 | T161 | 抖音短信与原设备二次验证、脱敏诊断和登录态恢复 |
