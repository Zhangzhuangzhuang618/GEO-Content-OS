...
2026-09-10 23:59生产完成：e5f89953fc929cf4e955fc639001205a350b1c94已部署并推送；16容器healthy、API/Web200、四镜像revision一致且重启0。两企业官网/列举网/企业介绍抖音6账号默认及日批均硬广，师傅/客户抖音4账号均standard；原口吻、定位、范围、地区、主题池、数量时间和启停深比较不变。主公司自动有效授权资料，其他公司可业务说明或明确沿用服务，不强制文件绑定、不共享证照/保险；四份说明生产解析active。已完成三平台各3篇隔离日批、自审及相关回归，新镜像只读核对10账号实际资料通过；补充生产等价生文复测被外发权限审核拦截，未计为通过。编译下载卡住后复用已验证依赖缓存、离线锁文件校验并重编译；没有重跑历史批次。完整验证、备份、回滚注意和部署详情见 `docs/release/T165-20260910-simple-evidence.md`。下方19:20及未部署记录为此前历史轮次。

2026-09-10 19:20生产完成：T165产品提交fd1334264d70ad1d84f018f6ea28314d91da9f3a已正常推送，API/Web/AI/Publisher四镜像更新、0060迁移成功；16容器healthy、API/Web200，新账号内容服务读取正常。三平台默认仍为standard，不自动配硬广名单；百家号两条策略已通过业务服务切independent，保留启停/日批数和时间、关闭派生专用fallback，2条审计确认。历史任务不重跑，用户文件和生产配置哈希不变。验收、备份、镜像与操作警告见 `docs/release/T165-20260910-account-editorial-policy.md`。下方未部署/继续测试描述为历史轮次，不代表当前状态。

2026-09-10 发布前收尾：T165三平台各3篇真实隔离日批及正文/图卡自审接受；发布门禁所有组成项通过（沙箱权限、临时恢复容器及E2E表数基线失败均保留记录并针对性重跑），不是一次完整命令全绿。E2E同步94张表并验证账号风格表存在。开始按用户条件授权执行生产备份与部署，部署结果以后续记录为准；不发布测试文章、不重跑历史任务、不自动切换账号硬广风格。

2026-09-10 18:58：T165最新隔离日批10-48轮完成三平台各3篇，共11候选、2个失败退出补位、96次模型调用；9篇正文、12项官网FAQ及21张抖音JPEG已完整自审，接受本轮可用性。删除图卡摘录阶段破坏正文语法的标点专用改写，缺短句走已有局部编辑；完善套餐实际项目重复和仓库无关业务目录检查。410项AI回归、Worker类型/构建通过；32页无障碍测试在沙箱外通过，本地恢复演练重跑通过；完整发布门禁其他检查仍在收尾。验收入口：`artifacts/t165-system-acceptance-2026-09-10T10-48-44-318Z/三平台日批验收.md`。所有测试配图为主动模板降级，不代表生产生图额度或服务健康。生产18:54核对16容器健康、API200，本地/远端/生产checkout仍aa5852，生产没有正在发布任务；一条9月2日遗留running生成记录不改。尚未提交/部署或切配置。

2026-09-10 18:00接续：用户已明确允许公开资料、经营者确认服务详情、证照文字和提示词发送api.deepseek.com，不发送原图、密码或客户资料。09-37、09-42、09-48、09-55轮均完成三平台各3篇排程，但自审发现标题关系、套餐选择、仓库误用家庭清单和照抄搜索词等问题，未作为最终验收。已修复总结卡合法模板回退、独立图卡摘句、拆装与日式档位关系、仓库目录和第二套套餐复述检查；410项AI测试、Worker类型/构建及静态发布门禁通过，API数据库集成34项通过。生产16容器healthy、API200；GitHub从服务器只读可连，远端HEAD与本地aa5852一致，本机HTTPS超时。继续隔离日批与自审，未提交/部署、未切配置。历史权限暂停不再是当前阻塞；详见T165最新记录。

2026-09-10 授权后续：用户已明确允许服务详情与证照文字随提示词发送api.deepseek.com，沙箱外隔离验证恢复。07-51、07-57两轮各9篇排程完成，但正文自审仍不全接受；修复改写遗漏分工计划、图卡只枚举1–2句却承诺1–3句、内部照片/证照说明泄露、补充回收抢占标题等问题，继续重跑。未部署、未切生产配置，以下权限暂停为历史记录。

2026-09-10 当前接续任务：持续修复、自审，再模拟官网/列举网/抖音各3篇日批，通过才部署。已本地修复质检JSON纠错丢项、图卡标题越界及布局选择、套餐问句误报、选材重试复制失败计划等问题，补充正文读者复核。07-30/07-33日批各9篇排程和幂等检查通过，但正文自审不全合格，继续修正后07-37轮仅5篇排程，存在模型请求/质检执行失败及内容改写耗尽，最终验收未通过。沙箱DeepSeek DNS失败，沙箱外只读连通检查成功；完整重跑被自动安全审核以经营者确认资料未获明确对外传输授权拒绝，已暂停外部调用，待用户明确允许将经营者确认服务详情与证照文字发送至api.deepseek.com；不涉及原图、密码、客户资料或业务真实性重新举证。原始日志保留，临时测试容器自动清理。外部发布/生图禁用，向量召回使用受限fixture，不代表真实发布或外部生图可用性。未部署、未切生产风格或百家号策略。进度以T165最新小节为准，以下为前轮历史记录。

2026-09-10 部署前最新复测：用户授权“重新生文，测试通过则部署”。三平台首次机器均pass，但自审未全部接受：列举网报告驱动改写后通过；官网两次质检报告JSON修复把已识别问题清空为pass，原稿仍有来源描述、重复清单等；抖音改写后正文改善但focus第3图卡长标题越过右边界。尚未修复，不部署。638项相关回归与静态发布检查通过不代表样稿通过。三个隔离数据库均导出后清理、发布任务0；生产HEAD与本地/远端均aa5852f7519d30ae3b6c17583576ddba9382f764，容器healthy/API200，仅只读核对。完整原文、FAQ、图卡与证据入口：`artifacts/t165-predeploy-test-20260910/三平台测试结果.md`。

T165 / ADR-0083：官网、列举网、抖音的账号默认生文风格与“硬广·多公司推荐”（功能初版与本地回归完成，真实内容质量仍待验收，未部署）。
账号配置有序的 2–6 家企业及各自资料；手动单次/日批策略/重发尝试可覆盖风格。批次与内容版本冻结配置，
资料在生成和发布前按租户、工作区、项目、权限、解析及有效期复核。其他平台仍走原生成规则。
迁移 0060 增加 1 张账号内容配置表及快照字段，当前开发基线为 94 张业务表、172 个公开端点。
百家号既有策略仍未切换；本版部署验收后再统一切为既有的独立生成模式，不自动重跑历史任务。

2026-09-10 最新单一需求改造：用户要求恢复早期样稿的具体需求、服务做法和用途写法，保留真实服务与证照；两家均说明完整搬迁身份，再各展开一个相关细节，回收短写为补充，保留实用 FAQ。初稿改为选材后全文一次生成，保留公司绑定引用及所有必要门禁。修复混合编辑意见导致框架回填旧稿、共同服务范围误判重复、补字丢失原写作要求/编辑意见、抖音选卡未传原质量意见。用户随后批准官网硬广改为正文约1100字（不含FAQ）的篇幅提示，取消该风格的正文最低字数门禁与字数补写，生成/编辑/质检提示同步；常规官网和其他平台长度规则不变。376项AI Worker及12项正文质检测试、相关类型/静态检查通过；原980字候选只读回放已不再触发官网硬广字数拦截，但未重新生成或进行模型语义质检，不代表三平台内容验收完成。此前仅列举网选定稿已自审接受，抖音最新试跑清单卡超长，不能沿用下方历史“已验收”状态。本轮仅本地、未部署，详见 T165 最新记录及 `artifacts/t165-length-hint-20260910/验证记录.md`。

2026-09-10 前轮证照改造：硬广推荐支持把对应公司绑定的有效授权证照自然写入该公司正文，企业主题最多三项、家庭主题一项；服务选材名额不被证照挤占，不重复追加推荐证照清单。检索、生成、发布均保持持证主体与引用归属校验；修复“该公司持有……”的扫描误判，发布兼容公司段内证照。未自动绑定生产资料，也未发送证照原图。
三平台真实生成、自审及必要改写完成，验收入口为 `artifacts/t165-certificate-acceptance-20260910/三平台证照版验收稿.md`（取代下方历史失败样稿作为本轮入口）。435 项相关测试及类型/静态检查通过；实际发布任务零，隔离数据库导出后清理。生产 HEAD、健康与相关日志仅只读核对，未提交/推送/部署、未改账号默认或日批，待用户业务验收，不外推长期成功率。

2026-09-10 本地真实验证使用隔离 PostgreSQL、实际 API 服务及 AI Worker，指定 DeepSeek 模型完成选材、正文、图卡、引用落库、质检及按报告改写。
新增原句选材/正文与图卡分阶段、质检原文引用输入；修复推荐引用提前清洗导致连续子串校验失败、抖音“A还是B/按需求选A或B”表达误判。
机器通过不等于人工合格：三公司抖音最新稿仍为 revise，官网仍有目录感，贵重物品对照稿存在能力延伸风险。没有修改生产、账号默认或日批。
验收原文、图卡、原始请求及人工意见见 `artifacts/t165-acceptance-20260910/三平台硬广验收稿与复核.md`，细节见 T165 任务记录。

2026-09-10 后续优化：采用负责人确认的企业搬迁环节和作业案例，资料按企业/主题分开；不公开共同经营、不强制企业差异化。
新增只编辑后续公司的精简步骤，首家/FAQ不被此步骤覆盖；取消平均篇幅指令；修复全局字数问题与自动局部保留冲突、重试基线未反映精简后正文的问题；选材引用ID加入可用枚举。
前轮样稿见 `artifacts/t165-acceptance-20260910-v2/三平台优化样稿与验收.md`。官网仍需改进，列举网/抖音机器pass但人工仍发现冗余或卡片题文不匹配；非全量达标。337项AI和176项Contracts测试通过，未部署、未切日批。

2026-09-10 最新本地优化：用“选材分工/冲突检查 → 各公司独立写作 → 冻结公司段后生成框架 → 摘录/标题/标题复核”替代后续公司事后精简。原文较长重复只修框架，标题只修失败卡；卡片修复耗尽不重写正文。明确的拆装额外收费可满足抖音卡片服务边界；总结卡按78px字号修复13汉字一行裁切，legacy布局不变。
最新同轮原文与复核见 `artifacts/t165-acceptance-20260910-v3/三平台系统原文与验收.md`。官网/列举网最终机器pass，抖音revise；人工仍未验收，存在回收重复、目录感、主次不稳及总质检误拿全文判断卡片的问题。未过滤质量意见强行放行。AI 343、Contracts 182、Image 40，共565项回归通过，Worker/API类型检查及相关ESLint通过。远端同分支HEAD与本地均为aa5852f7519d30ae3b6c17583576ddba9382f764，工作区变更未提交/推送/部署；生产账号/日批未改，临时验证数据库已导出并移除。

HOTFIX-20260907-03 修复 T155 资料上传对 JPEG 尾部附加数据的误判，并在资料图片尺寸和大小门禁后
执行完整解码，拒绝头部可读但像素数据截断的图片。原图字节、哈希、私有存储、安全扫描与授权不变；
无迁移、公开 API 或 Web 变化，只需更新 API，旧资料和失败请求不自动重试。

HOTFIX-20260907-02 修复 T164 抖音客户口吻的生成误判：收费对比标题要求明确比较收费的证据，
赠品金额不计入；扩展自然客户筛选与下一步表达；企业禁词前移到有界字段修复；统一首稿引用要求。
保留原质量、事实和禁词门禁，不补造缺失企业资料，不重跑历史失败；无迁移或公开 API 变化。

HOTFIX-20260907-01 补全 T162 的抖音登录用户名展示：迁移 0059 在抖音会话增加可空昵称字段，
既有账号列表及登录会话响应增加 account_nickname，PUB-01 分别展示自填账号名和实际抖音昵称。
昵称只读取当前账号已认证创作者中心首页的唯一账号头部，读取失败不改变认证状态；不作为 UID。
新扫码清空旧昵称，历史已登录账号首次核验时补采；表数与公开端点数保持不变。
...
### T165最新验收进展（2026-09-10，本地）

前述v1–v3记录为历史轮次。本轮持续修复后，三平台选定系统稿通过默认机器质检及逐篇自审，待用户业务验收。修复推荐主体/引入、照片来源叙述、按实际入库稿改写、未涉及公司段落保留、图卡服务摘录/标题/费用指代及长行溢出；质检不再注入不相关公众号示例，常规风格与事实门禁保留。
验收入口：`artifacts/t165-acceptance-20260910-v4/三平台硬广验收稿.md`。官网补修后单独重跑，列举网/抖音为原批次改写稿；均完整导出未手工润色，不称同轮成功率测试。正文、FAQ和7张抖音图卡已自审。
AI360、Contracts184、Image41、QualityCheckerSkill35共620项通过，最终Worker/API类型检查、相关ESLint及diff检查通过。隔离数据库已导出移除，发布任务0；未提交/推送/部署，未切账号默认或日批。本地及远端同分支HEAD均核实aa5852f7519d30ae3b6c17583576ddba9382f764。
...
### 企业基础资料与客户化正文（ADR-0078）

官网和列举网新文章从当前工作区已发布品牌资料识别唯一法定企业全称，并按主体精确匹配、有效期和
公开授权加载当前企业全部合格基础资料。基础资料不参与相似度排名、不占主题资料配额；每篇另取最多
3 份主题资料。官网增加独立“企业资质与保障”内容块，列举网增加一次简洁段落，企业名称、服务类型、
资料名称和完整引用映射均由服务端动态组装。只有工作区显式设置支持客户索取资料时才使用相应文案。
工作区还可声明必须具备的基础资料类别；配置后缺少任一类别即停止官网和列举网生成及发布，未配置时
使用当前企业实际拥有的全部合格资料。

所有平台的 Prompt、确定性扫描和发布层禁止把内部风控、证据分类、模型免责、资料 ID、`citation_map`
或内部规则名称写入客户正文；事实、主体、有效期、授权和引用校验仍在内部执行。名称缺失或冲突、工作区
显式要求的资料类别缺失、已有资料映射不完整或正文主体不一致时失败关闭；未配置必需类别且当前企业没有
可用基础资料时允许继续生成，但不得补造资质、认证、保险或赔付事实。旧内容不重写，其他工作区不得复用
当前企业资料。迁移 0054 只新增已发布 Prompt，不新增表或公开端点。
冻结 Seed 会在迁移后幂等补齐自动日批所需的 Quality Checker 1.2.0 和列举网 1.0.0 已发布规则，
确保全新数据库不会因历史迁移执行早于身份 Seed 而缺少运行基线。

HOTFIX-20260902-02 修正官网与列举网自动日批未采用当前已发布品牌 CTA 的问题：官网不再把 CTA 固定为
空值，列举网不再使用通用占位文案，两者均在创建候选时冻结最新已发布品牌资料中的 CTA；品牌 CTA 为空
时保持为空，不回退到退役版本或写死企业文案。搜狐号、抖音和百家号行为不变，无数据库或公开 API 变更。

### 抖音多账号内容隔离与资源控制（ADR-0079 / T162）

每个抖音自动化策略配置账号定位、服务范围、目标地区和主题池；每日候选确定性选出当次地区、服务与
主题，并把完整策略及选择结果冻结为标题、资料检索、受众和正文的强约束。历史空策略可完成迁移，但
新的抖音日批会失败关闭并提示补齐，搜狐号与列举网不使用这些字段。

`douyin_topic_reservations` 以租户内工作区作为公司边界，原子占用同日“关键词＋搜索意图”，防止多个
抖音账号生成相同搜索主题。抖音排期会读取同工作区其他抖音任务并以事务锁串行计算，冲突时按 20 分钟
顺延。Douyin Worker 继续使用 `douyin/{tenantId}/{accountId}` 独立 Profile，并把单实例发布与状态核验
限制为一次一个 Chromium 自动操作。迁移 0055 后当前业务表为 93 张，公开端点仍为 170 个；旧内容、
旧报告、旧任务和历史批次不重写或重排。

### 抖音账号级内容口吻与局部修复（ADR-0081 / T163）

每个抖音自动化策略新增独立内容口吻，当前允许“企业官方”和“一线师傅”。口吻不混入账号定位，候选
生成时单独冻结为 `brief.constraints.douyin_content_voice`：企业官方使用正式、克制的团队表达；一线
师傅使用通俗、直接的现场实务表达，以约十年现场经验的判断成熟度面对客户说明，但“十年”只是写作
标尺，不得在没有账号人物与工龄证据绑定时写成个人履历。师傅口吻必须包含通用第一人称工作方法和对
客户的直接提醒，不得切换成客户挑选服务商的身份；输入引用直接支持的官网公开案例或第三方经历可以
简洁转述并映射引用，但不得声称由叙述者亲手完成。

生成后的确定性门禁只把命中的抖音语义字段或正文块送去有界修复；缺少现场实务视角时只补一个未锁定
正文段，其他段落、事实和引用保持不变。迁移 0056 为现有抖音策略回填企业官方口吻并限制平台和值域，
不新增表或公开端点；当前业务表仍为 93 张，公开端点仍为 170 个，历史内容与批次不重写。

### 抖音客户口吻（ADR-0082 / T164）

抖音自动化策略的独立内容口吻新增 `customer_perspective`（客户口吻）。该口吻使用“我最担心……”“我会
先问清……”等自然第一人称，呈现潜在客户尚未成交时的真实顾虑和比较动作，但禁止“从客户视角”“作为
客户”等元话术。不得冒充已成交客户、编造消费亲历、客户评价或推荐结果，也不得模拟服务人员身份；输入
引用直接支持的第三方真实消费经历可以匿名概括并用于说明风险，但必须完成证据映射。

客户口吻仍用于推广当前企业：已发布品牌资料必须唯一识别法定企业全称，正文依据已发布品牌资料或输入
引用支持的本企业能力给出选择理由，资质等需外部证明的事实仍必须引用。正文首句由潜在客户自然进入
选择过程，至少写出一次真实的取舍，结尾以第一人称把本企业放进备选并说明下一步询价或核对方案后再
决定。主文案会合并为五个长短有变化的自然段；确定性门禁同时检查标题照抄、机械重复、角色元话术、
论文式或助手式套话、第三方案例引用和本企业推荐，命中时仍只修复对应字段；升级 Pro 前最多执行两轮
Flash 有界修复。正文逐字采用输入引用中的完整事实时由服务端确定性补齐引用映射，匿名第三方案例还须
与被引用原文逐字匹配；因结构错误已直接升级 Pro 时不会再次生成第二份 Pro 草稿。
迁移 0057 仅扩展既有抖音口吻值域，不修改现有策略，不新增表或公开端点；当前业务表仍为 93 张，公开
端点仍为 170 个，历史内容与批次不重写。

HOTFIX-20260903-01 修正三项浏览器平台可靠性问题：抖音多账号排期先遍历当天全部原始配置时段，再按
20 分钟逐轮错峰，避免首个冲突后把同一账号候选集中排在连续时段；百家号在旧 AI 声明控件缺失时仅接受
已知的新版“智能创作”编辑器签名，并把平台明确封停弹窗识别为账号限制；列举网自动日批在存在冻结引用
输入时必须采用至少一条与当前主题直接相关的引用，重写后仅对仍可见、引用仍获授权的原声明保留映射。
未知页面、错配引用和既有冻结质量门禁继续失败关闭；无数据库、公开 API、环境变量或 Compose 变化，
旧任务不自动重排或重写。

HOTFIX-20260903-02 修正抖音一线师傅口吻退化为中性教程或客户选择视角的问题：新稿须以成熟师傅面对
客户的说话方式写出通用第一人称现场做法，并自然使用“你”或“您”给出具体动作；师傅主文案合并为六
个长短不同的自然段；结尾由服务端使用品牌资料中的唯一法定企业全称，确定性组装为“我就职于当前企业，
欢迎联系我核对方案……”的任职与咨询表达，继续拦截自报工龄、编造姓名和虚构亲历。“约十年”仅作为
口吻成熟度标尺，不能由保险名单、企业成立年限或团队平均经验推导为叙述者个人工龄。已进入当前 Writer Input 的官网
公开案例或第三方经历可在引用直接支持并完成映射时使用，不得冒充本人经手；未导入资料库的网页不会
被自动抓取为生文事实。无数据库、公开 API、环境变量或 Compose 变化，旧内容和旧批次不重写。

HOTFIX-20260904-01 修正抖音日批的两类确定性故障：浏览器运行时报告 `Target crashed` 时，Worker 会释放
当前账号页面并仅重试一次；若该运行时故障已把会话标记为需人工处理，但仍保留加密登录态，下一次定时
发布会先安全复核登录态并恢复会话。CAPTCHA、短信验证、页面签名变化等真实人工挑战仍保持失败关闭。
日批短卡没有可见引用映射时，服务端可从当前唯一法定企业、主体精确匹配且已授权的结构化企业证照中
确定性补入一条短事实及引用；一线师傅缺少“你/您”时改为修复未锁定的正文段，不再错误修复随后必然被
服务端固定覆盖的结尾。质量分、事实校验、主体隔离和发布门禁均未降低，无数据库、公开 API、环境变量
或 Compose 变化，旧内容和旧批次不自动重写。

HOTFIX-20260904-02 修正百家号页面适配或浏览器运行时故障已修复后，旧的 `attention_required` 仍会在
打开浏览器前无条件阻断后续定时发布的问题。Worker 读取既有会话错误，仅对
`PAGE_SIGNATURE_CHANGED`、`EDITOR_OPERATION_FAILED` 和 `BROWSER_RUNTIME_FAILED` 执行一次发布就绪
复核；通过后刷新加密登录态并继续原发布流程。CAPTCHA、账号封停及其他人工状态不自动恢复；复核发现
平台封停时明确记录 `ACCOUNT_RESTRICTED` 并在创建发布记录前阻断。无数据库、公开 API、环境变量或
Compose 变化，历史终态任务不自动重试。

HOTFIX-20260905-01 修正三项日批确定性校验缺陷：百家号、列举网和搜狐的质量重写仍向模型提供去除
服务端字段的精简候选，但“内容无变化”改用精简前的完整当前版本作为比较基线；详细地址识别改为要求
同一句段内出现明确门牌、道路、街巷、大厦或园区地址特征，不再把地址清单、搬运路线、道路/街道或
园区进场规则拼成具体地址，并支持带多级逗号的真实地址；抖音从授权结构化证照自动补入短事实时使用
“该企业”自然指代，不再额外重复法定企业全称。原有证照主体、授权和引用校验，抖音企业名称最多 2 次、
解决方案段提及、师傅固定结尾、客户备选结尾及全部质量门槛不变。无数据库、公开 API、环境变量或
Compose 变化，历史内容、质量报告和批次不自动重写或重试，只需重建 ai-worker。

HOTFIX-20260905-02 基于 12:00 后人工抖音链路修正登录态误判：页面认证由布尔值改为认证、明确登录、
未知三态，只有明确登录控件或登录 pathname 才判定失效；未知页面返回 `PAGE_SIGNATURE_CHANGED` 并
保持 `attention_required`。已有完整持久化登录态但内存验证页已释放时，状态轮询重新打开独立 Profile
核验，成功后恢复认证，明确进入登录页才写 `LOGIN_EXPIRED`/`reauth`。生产 Creator Center 已确认
图文标题上限为 20 字，因此写作提示、契约、确定性门禁、日批标题、渲染载荷和浏览器网关统一为 20 字，
迁移 0058 发布 `douyin@1.2.0` 动态规则。旧规则和历史记录保留，既有失败或超长排期不自动改写重试；
需重建 api、ai-worker、publisher-worker、douyin-browser 并执行迁移，无公开 API、环境变量或 Compose 变化。
...
HOTFIX-20260830-12 将抖音日批从每天重置的通用流程型选题改为按业务日期和候选序号轮换的十二类搜索
决策意图，覆盖推荐选择、方案比较、收费、正规性、合同、赔付、车型、人工、楼层与进场、避坑、工期
和验收。候选标题在 26 字内保留决策问题，受限 Flash 草稿必须沿用服务端绑定的唯一意图，现场、报价、
防护、工期和清单槽位只解释该主题的条件与边界，不再形成每篇相同的平行结构。“真实场景、收费对比、
资质核验、合同条款解读、口碑、榜单”等标题证据承诺必须在可见正文中有对应 citation_map；否则改为
核对方法、选择标准或比较维度。冻结事实、企业名称、资质、抖音编辑和自动化质量门槛不变；无数据库
迁移、公开 API、新环境变量或其他平台生成变化，旧内容和旧批次不自动重写。

HOTFIX-20260831-01 修复抖音受限草稿在证据元数据和确定性字段修复上的两个闭环缺口。模型返回的
evidence_claims 若引用已提供但 claim_text 不是可见文案的逐字子串，原流程只能整体升级 Pro，无法只修复
证据映射；现在增加无工具、只允许改 evidence_claims 的受限修复，服务端仍逐项验证引用 ID、可见原文和
全部资质/标题证据门禁。opening_pain 提示和定点修复明确要求写出具体对象及现实问题或后果，并包含冻结
检测词之一；Pro 若仍留下长度或字面规则问题，再执行一次受限 Flash 字段修复。不得把无效映射作为
通过依据、放行无证据声明或降低任何质量阈值。无迁移、公开 API、环境变量或 Compose 变化，只需重建
ai-worker。

HOTFIX-20260831-02 修复抖音受限草稿在输入已经提供引用时仍允许空 `evidence_claims` 进入独立质量检查的
缺口。生产复验中，志远搬家 8 个未合格候选有 6 个以空 `citation_map` 进入质量检查，其确定性证据分均为
55，质量报告又没有返回具体问题位置，后续重写无法定位事实声明并多次原样返回。现在只要当前输入包含
引用，受限草稿就必须至少建立一条“可见完整声明—已提供引用”的直接支持映射；先走既有证据元数据受限
修复，无法从当前文案建立有效映射时才升级完整 Pro 草稿修订。引用 ID、可见原文、资质证照、标题证据
承诺、独立事实评分及全部冻结门槛继续由原服务端检查，不按引用数量加分，也不伪造或强配引用。无迁移、
公开 API、环境变量或 Compose 变化，只需重建 ai-worker。

HOTFIX-20260831-03 修复抖音发布前登录态预检复用已崩溃 Playwright 页面、导致任务的自动重试持续返回
`Page crashed` 的缺口。生产 08:00 两个账号的容器内存事件共记录两次 `oom_kill`，随后两条任务各连续
失败三次；数据库没有 `submitted_at`，可确认尚未点击发布。预检现在只对明确的页面、上下文或浏览器崩溃
执行一次受限恢复：关闭该账号上下文，以原独立 Profile 和已保存登录态重新打开并复核；第二次失败保持
原失败语义，不循环重试或伪判成功。日志只保留账号 ID 和脱敏运行时错误。无迁移、公开 API、环境变量、
Compose、发布幂等或质量门禁变化，只需重建 douyin-browser。

HOTFIX-20260831-04 修复抖音自动发布和非终态对账完成后仍长期保留每账号 Playwright 持久上下文的问题。
生产复验中，第一条替代任务提交后 douyin-browser 常驻内存约 1.0 GiB，而 Docker 虚拟机仅余约 1.2 GiB
可用内存且交换空间已耗尽；继续打开第二账号或跨账号对账存在再次 OOM 的确定风险。自动发布现在无论
成功或失败都在账号锁释放前关闭上下文，非终态自动对账完成后同样关闭；独立 Profile、Cookie 快照、
发布记录和证据均继续持久化，后续按原 Profile 重开。等待人工二次验证的活动页面和终态只读查询不变，
关闭失败只记录脱敏错误且不覆盖发布结果。无迁移、公开 API、环境变量、Compose、尝试上限或质量门禁
变化，只需重建 douyin-browser。

HOTFIX-20260831-05 修复抖音日批“地域＋具体场景＋决策问题”和标题证据承诺仍只依赖提示词、没有服务端
验收的缺口。系统现在从关键词导入元数据和项目主地域绑定标题主体，生成及重写必须逐字保留；检索证据
与当前决策意图明确匹配时，再绑定资质核验、合同条款解读、收费对比或真实场景，标题、正文可见事实与
引用映射必须共同兑现。确定性门禁继续保护所有既有质量阈值和无证据声明边界。当天候选编辑序号跨批次
尝试累计，补位不再从首个主题重新轮换。无迁移、公开 API、环境变量或 Compose 变化，只需重建
ai-worker；生产只撤销尚未发布且没有发布尝试或远端记录的不合格排期，保留已发布作品及全部审计历史。
...
| M23 | T162 | 抖音账号定位、公司级同日选题占位、发布错峰与单实例资源控制 |
| M24 | T163 | 抖音账号级企业官方/一线师傅口吻、确定性拦截与局部修复 |
| M25 | T164 | 抖音客户决策口吻、虚构消费经历拦截与局部修复 |
