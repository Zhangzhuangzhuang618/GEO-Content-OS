...
HOTFIX-20260819-06 将证照扫描件从普通配图门槛中分离：证照输入允许最长边 8192 像素、总像素不超过 5000 万且文件不超过 10 MB，原图继续私有保存；随文发布仍重新编码为完整画面、不裁切、移除元数据的 1200×800 JPEG。普通生成图片的 4096 像素门槛、冻结质量门槛、旧资料、旧内容和旧质量报告均不变，无数据库迁移。
