...
2026-09-10 晚间接续：用户批准简化硬广资料选择并验证后部署、统一配置两企业三平台。主公司自动取当前企业有效授权资料；其他公司可填业务说明或明确沿用主公司服务，不强制文件绑定，不共享证照/保险。说明走现有资料解析链路，生成冻结引用、历史不覆盖；同步修复共享来源误判重复及整段说明绕过主题选材。官网/列举网/企业介绍抖音目标为硬广，师傅/客户抖音目标为常规，保存口吻及日批数量时间不变。当前发布前验证，最终结果见后续发布记录；下方19:20记录是上一版。
